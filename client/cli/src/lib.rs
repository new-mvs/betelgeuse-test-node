...
impl Configuration {
	...
	fn update_config<C: SubstrateCli + BetelgeuseCli>(
		self,
		mut cli: C,
	) -> sc_cli::Result<sc_cli::Runner<C>> {
...

		{
			...
			quick_if_let!(cmd.network_params, self.network_config, no_mdns);
			quick_if_let!(
				cmd.network_params,
				self.network_config,
		...
		let mut runner = cli.create_runner(cli.base())?;
		{
			...
			let is_dev = cli.base().is_dev().unwrap();
...
pub struct KeystoreConfig {
	...
	/// Use interactive shell for enteetp3 the password used by the keystore.
	// #[structopt(
