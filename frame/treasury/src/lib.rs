...
//!
//! Tipping protocol:
//! - **Tipping:** The process of gatheetp3 declarations of amounts to tip and taking the median
...
mod types {
	...
	pub type Etp3Balance<T, I> = <Etp3Currency<T, I> as Currency<AccountId<T>>>::Balance;
	pub type Etp3PositiveImbalance<T, I> =
		<Etp3Currency<T, I> as Currency<AccountId<T>>>::PositiveImbalance;
	pub type Etp3NegativeImbalance<T, I> =
		<Etp3Currency<T, I> as Currency<AccountId<T>>>::NegativeImbalance;
	...
	type Etp3Currency<T, I> = <T as Trait<I>>::Etp3Currency;
	type DnaCurrency<T, I> = <T as Trait<I>>::DnaCurrency;
}
...
use sp_std::prelude::*;
// --- betelgeuse ---
use betelgeuse_support::balance::{lock::LockableCurrency, OnUnbalancedDna};
use types::*;
...
pub trait Trait<I = DefaultInstance>: frame_system::Trait {
	...
	type TipReportDepositBase: Get<Etp3Balance<Self, I>>;

	/// The amount held on deposit per byte within the tip report reason or bounty description.
	type DataDepositPerByte: Get<Etp3Balance<Self, I>>;

	/// The overarching event type.
	type Event: From<Event<Self, I>> + Into<<Self as frame_system::Trait>::Event>;

	/// Handler for the unbalanced decrease when slashing for a rejected proposal or bounty.
	type OnSlashEtp3: OnUnbalanced<Etp3NegativeImbalance<Self, I>>;
	/// Handler for the unbalanced decrease when slashing for a rejected proposal or bounty.
	...
	type Etp3ProposalBondMinimum: Get<Etp3Balance<Self, I>>;
	...
	type BountyDepositBase: Get<Etp3Balance<Self, I>>;
	...
	type BountyValueMinimum: Get<Etp3Balance<Self, I>>;

	/// Maximum acceptable reason length.
	type MaximumReasonLength: Get<u32>;

	/// Handler for the unbalanced decrease when treasury funds are burned.
	type Etp3BurnDestination: OnUnbalanced<Etp3NegativeImbalance<Self, I>>;
...
decl_storage! {
	trait Store for Module<T: Trait<I>, I: Instance = DefaultInstance> as BetelgeuseTreasury {
		...
		Proposals
			...
			=> Option<TreasuryProposal<T::AccountId, Etp3Balance<T, I>, DnaBalance<T, I>>>;
		...
		pub Tips
			...
			=> Option<OpenTip<T::AccountId, Etp3Balance<T, I>, T::BlockNumber, T::Hash>>;
		...
		pub Bounties
			...
			=> Option<Bounty<T::AccountId, Etp3Balance<T, I>, T::BlockNumber>>;
	...
	add_extra_genesis {
		build(|_config| {
			...
			{
				let min = T::Etp3Currency::minimum_balance();

				if T::Etp3Currency::free_balance(&account_id) < min {
...
decl_event!(
	...
	where
		...
		Etp3Balance = Etp3Balance<T, I>,
		DnaBalance = DnaBalance<T, I>,
	{
		/// New proposal. [proposal_index]
		Proposed(ProposalIndex),
		/// We have ended a spend period and will now allocate funds. [budget_remaining_etp3]
		Spending(Etp3Balance, DnaBalance),
		/// Some funds have been allocated. [proposal_index, award, beneficiary]
		Awarded(ProposalIndex, Etp3Balance, DnaBalance, AccountId),
		/// A proposal was rejected; funds were slashed. [proposal_index, slashed]
		Rejected(ProposalIndex, Etp3Balance, DnaBalance),
		/// Some of our funds have been burnt. [burn]
		Burnt(Etp3Balance, DnaBalance),
		/// Spending has finished; this is the amount that rolls over until next spend. [budget_remaining_etp3]
		Rollover(Etp3Balance, DnaBalance),
		/// Some *RING* have been deposited. [deposit]
		DepositEtp3(Etp3Balance),
...
decl_module! {
		...
		origin: T::Origin
	{
		...
		/// Minimum amount of *RING* that should be placed in a deposit for making a proposal.
		const Etp3ProposalBondMinimum: Etp3Balance<T, I> = T::Etp3ProposalBondMinimum::get();
		/// Minimum amount of *KTON* that should be placed in a deposit for making a proposal.
		...
		/// The amount held on deposit for placing a tip report.
		const TipReportDepositBase: Etp3Balance<T, I> = T::TipReportDepositBase::get();

		/// The amount held on deposit per byte within the tip report reason or bounty description.
		const DataDepositPerByte: Etp3Balance<T, I> = T::DataDepositPerByte::get();

		/// The treasury's module id, used for deriving its sovereign account ID.
		const ModuleId: ModuleId = T::ModuleId::get();

		/// The amount held on deposit for placing a bounty proposal.
		const BountyDepositBase: Etp3Balance<T, I> = T::BountyDepositBase::get();
		...
		const BountyValueMinimum: Etp3Balance<T, I> = T::BountyValueMinimum::get();
		...
		fn propose_spend(
			origin,
			#[compact] etp3_value: Etp3Balance<T, I>,
			...
			let etp3_bond = Self::calculate_bond::<_, T::Etp3ProposalBondMinimum>(etp3_value);
			let dna_bond = Self::calculate_bond::<_, T::DnaProposalBondMinimum>(dna_value);

			T::Etp3Currency::reserve(&proposer, etp3_bond)
		...
		fn reject_proposal(origin, #[compact] proposal_id: ProposalIndex) {
			...
			let etp3_bond = proposal.etp3_bond;
			let imbalance_etp3 = T::Etp3Currency::slash_reserved(&proposal.proposer, etp3_bond).0;
			T::OnSlashEtp3::on_unbalanced(imbalance_etp3);
			...
			Self::deposit_event(<Event<T, I>>::Rejected(proposal_id, etp3_bond, dna_bond));
		}
		...
		fn report_awesome(origin, reason: Vec<u8>, who: T::AccountId) {
			...
			T::Etp3Currency::reserve(&finder, deposit)?;
		...
		fn retract_tip(origin, hash: T::Hash) {
			...
			if !tip.deposit.is_zero() {
				let _ = T::Etp3Currency::unreserve(&who, tip.deposit);
			}
		...
		fn tip_new(origin, reason: Vec<u8>, who: T::AccountId, #[compact] tip_value: Etp3Balance<T, I>) {
		...
		fn tip(origin, hash: T::Hash, #[compact] tip_value: Etp3Balance<T, I>) {
			let tipper = ensure_signed(origin)?;
			ensure!(T::Tippers::contains(&tipper), BadOrigin);
		...
		fn unassign_curator(
			...
			<Bounties<T, I>>::try_mutate_exists(bounty_id, |maybe_bounty| -> DispatchResult {
				...
				let slash_curator = |curator: &T::AccountId, curator_deposit: &mut Etp3Balance<T, I>| {
					let imbalance = T::Etp3Currency::slash_reserved(curator, *curator_deposit).0;
					T::OnSlashEtp3::on_unbalanced(imbalance);
						...
						match maybe_sender {
							...
							Some(sender) => {
								...
								if sender != *curator {
									...
									let _ = T::Etp3Currency::unreserve(&curator, bounty.curator_deposit);
									// Continue to change bounty status below...
								}
							},
						}
			...
			<Bounties<T, I>>::try_mutate_exists(bounty_id, |maybe_bounty| -> DispatchResult {
				...
				match bounty.status {
					BountyStatus::CuratorProposed { ref curator } => {
						...
						T::Etp3Currency::reserve(curator, deposit)?;
		...
		fn claim_bounty(origin, #[compact] bounty_id: BountyIndex) {
			...
			<Bounties<T, I>>::try_mutate_exists(bounty_id, |maybe_bounty| -> DispatchResult {
				...
				if let BountyStatus::PendingPayout { curator, beneficiary, unlock_at } = bounty.status {
					...
					let balance = T::Etp3Currency::free_balance(&bounty_account);
					let fee = bounty.fee.min(balance); // just to be safe
					let payout = balance.saturating_sub(fee);
					let _ = T::Etp3Currency::unreserve(&curator, bounty.curator_deposit);
					let _ = T::Etp3Currency::transfer(&bounty_account, &curator, fee, AllowDeath); // should not fail
					let _ = T::Etp3Currency::transfer(&bounty_account, &beneficiary, payout, AllowDeath); // should not fail
					*maybe_bounty = None;
			...
			<Bounties<T, I>>::try_mutate_exists(bounty_id, |maybe_bounty| -> DispatchResultWithPostInfo {
				...
				match &bounty.status {
					BountyStatus::Proposed => {
						...
						let imbalance = T::Etp3Currency::slash_reserved(&bounty.proposer, value).0;
						T::OnSlashEtp3::on_unbalanced(imbalance);
						*maybe_bounty = None;
				...
				let balance = T::Etp3Currency::free_balance(&bounty_account);
				let _ = T::Etp3Currency::transfer(&bounty_account, &Self::account_id(), balance, AllowDeath); // should not fail
				*maybe_bounty = None;
...
impl<T: Trait<I>, I: Instance> Module<T, I> {
	...
	fn create_bounty(
		...
		T::Etp3Currency::reserve(&proposer, bond)
			.map_err(|_| <Error<T, I>>::InsufficientProposersBalance)?;
	...
	fn insert_tip_and_check_closing(
		tip: &mut OpenTip<T::AccountId, Etp3Balance<T, I>, T::BlockNumber, T::Hash>,
		tipper: T::AccountId,
		tip_value: Etp3Balance<T, I>,
	) -> bool {
	...
	fn retain_active_tips(tips: &mut Vec<(T::AccountId, Etp3Balance<T, I>)>) {
	...
	fn payout_tip(
		hash: T::Hash,
		tip: OpenTip<T::AccountId, Etp3Balance<T, I>, T::BlockNumber, T::Hash>,
	) {
		let mut tips = tip.tips;
		Self::retain_active_tips(&mut tips);
		tips.sort_by_key(|i| i.1);
		let treasury = Self::account_id();
		let max_payout = Self::pot::<T::Etp3Currency>();
		let mut payout = tips[tips.len() / 2].1.min(max_payout);
		if !tip.deposit.is_zero() {
			let _ = T::Etp3Currency::unreserve(&tip.finder, tip.deposit);
		}
		if tip.finders_fee {
			if tip.finder != tip.who {
				...
				let _ = T::Etp3Currency::transfer(&treasury, &tip.finder, finders_fee, KeepAlive);
			}
		}
		// same as above: best-effort only.
		let _ = T::Etp3Currency::transfer(&treasury, &tip.who, payout, KeepAlive);
	...
	fn spend_funds() -> Weight {
		...
		let mut budget_remaining_etp3 = Self::pot::<T::Etp3Currency>();
		let mut budget_remaining_dna = Self::pot::<T::DnaCurrency>();

		Self::deposit_event(RawEvent::Spending(
			budget_remaining_etp3,
			budget_remaining_dna,
		));

		let mut missed_any_etp3 = false;
		let mut imbalance_etp3 = <Etp3PositiveImbalance<T, I>>::zero();
		...
		let proposals_len = <Approvals<I>>::mutate(|v| {
			...
			v.retain(|&index| {
				...
				if let Some(p) = Self::proposals(index) {
					if p.etp3_value > budget_remaining_etp3 || p.dna_value > budget_remaining_dna
					{
						if p.etp3_value > budget_remaining_etp3 {
							missed_any_etp3 = true;
						}
					...
					if p.etp3_value <= budget_remaining_etp3 {
						budget_remaining_etp3 -= p.etp3_value;

						// return their deposit.
						let _ = T::Etp3Currency::unreserve(&p.proposer, p.etp3_bond);

						// provide the allocation.
						imbalance_etp3.subsume(T::Etp3Currency::deposit_creating(
							&p.beneficiary,
							p.etp3_value,
						));
					}
			...
			v.retain(|&index| {
				<Bounties<T, I>>::mutate(index, |bounty| {
					...
					if let Some(bounty) = bounty {
						if bounty.value <= budget_remaining_etp3 {
							budget_remaining_etp3 -= bounty.value;

							bounty.status = BountyStatus::Funded;

							// return their deposit.
							let _ = T::Etp3Currency::unreserve(&bounty.proposer, bounty.bond);
		...
		{
			let burn_etp3 = if !missed_any_etp3 {
				// burn some proportion of the remaining budget if we run a surplus.
				let burn = (T::Burn::get() * budget_remaining_etp3).min(budget_remaining_etp3);
				budget_remaining_etp3 -= burn;

				let (debit, credit) = T::Etp3Currency::pair(burn);
				imbalance_etp3.subsume(debit);
				T::Etp3BurnDestination::on_unbalanced(credit);
			...
			Self::deposit_event(RawEvent::Burnt(burn_etp3, burn_dna));
		}

		// Must never be an error, but better to be safe.
		// proof: budget_remaining_etp3 is account free balance minus ED;
		// Thus we can't spend more than account free balance minus ED;
		// Thus account is kept alive; qed;
		if let Err(problem) = T::Etp3Currency::settle(
			&Self::account_id(),
			imbalance_etp3,
...
impl<T: Trait<I>, I: Instance> OnUnbalanced<Etp3NegativeImbalance<T, I>> for Module<T, I> {
	fn on_nonzero_unbalanced(amount: Etp3NegativeImbalance<T, I>) {
		let numeric_amount = amount.peek();

		// Must resolve into existing but better to be safe.
		let _ = T::Etp3Currency::resolve_creating(&Self::account_id(), amount);

		Self::deposit_event(RawEvent::DepositEtp3(numeric_amount));
	}
}
// FIXME: Ugly hack due to https://github.com/rust-lang/rust/issues/31844#issuecomment-557918823
impl<T: Trait<I>, I: Instance> OnUnbalancedDna<DnaNegativeImbalance<T, I>> for Module<T, I> {
...
#[derive(Encode, Decode, Clone, PartialEq, Eq, RuntimeDebug)]
pub struct TreasuryProposal<AccountId, Etp3Balance, DnaBalance> {
	/// The account proposing it.
	proposer: AccountId,
	/// The account to whom the payment should be made if the proposal is accepted.
	beneficiary: AccountId,
	/// The (total) *RING* that should be paid if the proposal is accepted.
	etp3_value: Etp3Balance,
...
> {
	...
	tips: Vec<(AccountId, Etp3Balance)>,
