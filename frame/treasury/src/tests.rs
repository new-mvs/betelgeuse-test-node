...
fn tip_new_cannot_be_used_twice() {
	new_test_ext().execute_with(|| {
		Etp3::make_free_balance_be(&Treasury::account_id(), 101);
		assert_ok!(Treasury::tip_new(
			Origin::signed(10),
			b"awesome.betelgeuse".to_vec(),
			3,
			10
		));
		assert_noop!(
			Treasury::tip_new(Origin::signed(11), b"awesome.betelgeuse".to_vec(), 3, 10),
...
fn report_awesome_and_tip_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::reserved_balance(0), 17);
		assert_eq!(Etp3::free_balance(0), 83);

		// other reports don't count.
		assert_noop!(
			Treasury::report_awesome(Origin::signed(1), b"awesome.betelgeuse".to_vec(), 3),
			<Error<Test, _>>::AlreadyKnown
		);
		...
		assert_eq!(Etp3::reserved_balance(0), 0);
		assert_eq!(Etp3::free_balance(0), 102);
		assert_eq!(Etp3::free_balance(3), 8);
	});
}
...
fn report_awesome_from_beneficiary_and_tip_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::reserved_balance(0), 17);
		assert_eq!(Etp3::free_balance(0), 83);
		let h = BlakeTwo256::hash_of(&(BlakeTwo256::hash(b"awesome.betelgeuse"), 0u128));
		assert_ok!(Treasury::tip(Origin::signed(10), h.clone(), 10));
		assert_ok!(Treasury::tip(Origin::signed(11), h.clone(), 10));
		assert_ok!(Treasury::tip(Origin::signed(12), h.clone(), 10));
		System::set_block_number(2);
		assert_ok!(Treasury::close_tip(Origin::signed(100), h.into()));
		assert_eq!(Etp3::reserved_balance(0), 0);
		assert_eq!(Etp3::free_balance(0), 110);
	});
}
...
fn close_tip_works() {
	new_test_ext().execute_with(|| {
		...
		Etp3::make_free_balance_be(&Treasury::account_id(), 101);
		assert_eq!(Treasury::pot::<Etp3>(), 100);
		...
		assert_eq!(Etp3::free_balance(3), 10);
...
fn spend_proposal_takes_min_deposit() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(0), 99);
		assert_eq!(Etp3::reserved_balance(0), 1);
	});
}
...
fn spend_proposal_takes_proportional_deposit() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(0), 95);
		assert_eq!(Etp3::reserved_balance(0), 5);
	});
}
...
fn accepted_spend_proposal_ignored_outside_spend_period() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(3), 0);
		assert_eq!(Treasury::pot::<Etp3>(), 100);
	});
}
...
fn unused_pot_should_diminish() {
	new_test_ext().execute_with(|| {
		...
		Etp3::make_free_balance_be(&Treasury::account_id(), 101);
		assert_eq!(Etp3::total_issuance(), init_total_issuance + 100);

		<Treasury as OnInitialize<u64>>::on_initialize(2);
		assert_eq!(Treasury::pot::<Etp3>(), 50);
		assert_eq!(Etp3::total_issuance(), init_total_issuance + 50);
	});
}
...
fn accepted_spend_proposal_enacted_on_spend_period() {
	new_test_ext().execute_with(|| {
		...
		<Treasury as OnInitialize<u64>>::on_initialize(2);
		assert_eq!(Etp3::free_balance(3), 100);
		assert_eq!(Treasury::pot::<Etp3>(), 0);
	});
}
...
fn pot_underflow_should_not_diminish() {
	new_test_ext().execute_with(|| {
		...
		<Treasury as OnInitialize<u64>>::on_initialize(2);
		assert_eq!(Treasury::pot::<Etp3>(), 100); // Pot hasn't changed

		let _ = Etp3::deposit_into_existing(&Treasury::account_id(), 100).unwrap();
		<Treasury as OnInitialize<u64>>::on_initialize(4);
		assert_eq!(Etp3::free_balance(3), 150); // Fund has been spent
		assert_eq!(Treasury::pot::<Etp3>(), 25); // Pot has finally changed
	});
}
...
fn treasury_account_doesnt_get_deleted() {
	new_test_ext().execute_with(|| {
		...
		let treasury_balance = Etp3::free_balance(&Treasury::account_id());
		...
		<Treasury as OnInitialize<u64>>::on_initialize(4);
		assert_eq!(Treasury::pot::<Etp3>(), 0); // Pot is emptied
		assert_eq!(Etp3::free_balance(Treasury::account_id()), 1); // but the account is still there
	});
}
...
fn inexistent_account_works() {
	...
	t.execute_with(|| {
		assert_eq!(Etp3::free_balance(Treasury::account_id()), 0); // Account does not exist
		assert_eq!(Treasury::pot::<Etp3>(), 0); // Pot is empty
		assert_ok!(Treasury::propose_spend(Origin::signed(0), 99, 0, 3));
		assert_ok!(Treasury::approve_proposal(Origin::root(), 0));
		assert_ok!(Treasury::propose_spend(Origin::signed(0), 1, 0, 3));
		assert_ok!(Treasury::approve_proposal(Origin::root(), 1));
		<Treasury as OnInitialize<u64>>::on_initialize(2);
		assert_eq!(Treasury::pot::<Etp3>(), 0); // Pot hasn't changed
		assert_eq!(Etp3::free_balance(3), 0); // Balance of `3` hasn't changed

		Etp3::make_free_balance_be(&Treasury::account_id(), 100);
		assert_eq!(Treasury::pot::<Etp3>(), 99); // Pot now contains funds
		assert_eq!(Etp3::free_balance(Treasury::account_id()), 100); // Account does exist
		<Treasury as OnInitialize<u64>>::on_initialize(4);
		assert_eq!(Treasury::pot::<Etp3>(), 0); // Pot has changed
		assert_eq!(Etp3::free_balance(3), 99); // Balance of `3` has changed
	});
}
...
fn propose_bounty_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::reserved_balance(0), deposit);
		assert_eq!(Etp3::free_balance(0), 100 - deposit);
...
fn approve_bounty_works() {
	new_test_ext().execute_with(|| {
		...
		// return deposit
		assert_eq!(Etp3::reserved_balance(0), 0);
		assert_eq!(Etp3::free_balance(0), 100);
		...
		assert_eq!(Treasury::pot::<Etp3>(), 100 - 50 - 25); // burn 25
		assert_eq!(Etp3::free_balance(Treasury::bounty_account_id(0)), 50);
	});
}
...
fn assign_curator_works() {
	new_test_ext().execute_with(|| {
		...
		assert_noop!(
			...
			<betelgeuse_balances::Error<Test, Etp3Instance>>::InsufficientBalance
		);

		Etp3::make_free_balance_be(&4, 10);
		...
		assert_eq!(Etp3::free_balance(&4), 8);
		assert_eq!(Etp3::reserved_balance(&4), 2);
	});
}
...
fn unassign_curator_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(&4), 8);
		assert_eq!(Etp3::reserved_balance(&4), 0); // slashed 2
	});
}
...
fn award_and_claim_bounty_works() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(4), 8); // inital 10 - 2 deposit
		...
		assert_eq!(Etp3::free_balance(4), 14); // initial 10 + fee 4
		assert_eq!(Etp3::free_balance(3), 56);
		assert_eq!(Etp3::free_balance(Treasury::bounty_account_id(0)), 0);
...
fn claim_handles_high_fee() {
	new_test_ext().execute_with(|| {
		System::set_block_number(1);
		Etp3::make_free_balance_be(&Treasury::account_id(), 101);
		Etp3::make_free_balance_be(&4, 30);
		assert_ok!(Treasury::propose_bounty(
			Origin::signed(0),
			50,
			b"12345".to_vec()
		...
		// make fee > balance
		let _ = Etp3::slash(&Treasury::bounty_account_id(0), 10);

		assert_ok!(Treasury::claim_bounty(Origin::signed(1), 0));

		assert_eq!(last_event(), RawEvent::BountyClaimed(0, 0, 3));

		assert_eq!(Etp3::free_balance(4), 70); // 30 + 50 - 10
		assert_eq!(Etp3::free_balance(3), 0);
		assert_eq!(Etp3::free_balance(Treasury::bounty_account_id(0)), 0);
...
fn cancel_and_refund() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(Treasury::bounty_account_id(0)), 60);

		assert_noop!(Treasury::close_bounty(Origin::signed(0), 0), BadOrigin);

		assert_ok!(Treasury::close_bounty(Origin::root(), 0));

		assert_eq!(Treasury::pot::<Etp3>(), 85); // - 25 + 10
	});
}
...
fn expire_and_unassign() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(1), 93);
		assert_eq!(Etp3::reserved_balance(1), 5);
		...
		assert_eq!(Etp3::free_balance(1), 93);
		assert_eq!(Etp3::reserved_balance(1), 0); // slashed
	});
}
...
fn extend_expiry() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(4), 5);
		assert_eq!(Etp3::reserved_balance(4), 5);
		...
		assert_eq!(Etp3::free_balance(4), 10); // not slashed
		assert_eq!(Etp3::reserved_balance(4), 0);
	});
}
...
fn approve_proposal_no_keep_burning() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(&1), 98);
		...
		<Treasury as OnInitialize<u64>>::on_initialize(1);
		assert_eq!(Etp3::free_balance(&0), 95); // ProposalBond: Permill::from_percent(5);
		assert_eq!(Dna::free_balance(&0), 95); // ProposalBond: Permill::from_percent(5);
		assert_eq!(Etp3::free_balance(&1), 98); // No changes
		assert_eq!(Dna::free_balance(&1), 98); // No changes
		assert_eq!(Etp3::free_balance(&2), 1); // No changes
		assert_eq!(Dna::free_balance(&2), 1); // No changes
		assert_eq!(Etp3::free_balance(&3), 0); // No changes
		assert_eq!(Dna::free_balance(&3), 0); // No changes
		assert_eq!(Treasury::pot::<Etp3>(), 100); // No changes
		...
		assert_eq!(Etp3::free_balance(&0), 100); // ProposalBond: Permill::from_percent(5); **return bond**
		assert_eq!(Dna::free_balance(&0), 100); // ProposalBond: Permill::from_percent(5); **return bond**
		assert_eq!(Etp3::free_balance(&1), 98); // No changes
		assert_eq!(Dna::free_balance(&1), 98); // No changes
		assert_eq!(Etp3::free_balance(&2), 1); // No changes
		assert_eq!(Dna::free_balance(&2), 1); // No changes
		assert_eq!(Etp3::free_balance(&3), 100); // No changes
		assert_eq!(Dna::free_balance(&3), 100); // No changes
		assert_eq!(Treasury::pot::<Etp3>(), 0); // Burn: Permill::from_percent(50); **Burn 100 if approve**
		...
		assert_eq!(Etp3::free_balance(&0), 100); // No changes from last perid
		...
		assert_eq!(Etp3::free_balance(&3), 100); // No changes
		assert_eq!(Dna::free_balance(&3), 100); // No changes
		assert_eq!(Treasury::pot::<Etp3>(), 0); // No changes from last perid
...
fn reject_proposal_keep_burning() {
	new_test_ext().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(&3), 0); // No changes
		assert_eq!(Dna::free_balance(&3), 0); // No changes
		assert_eq!(Treasury::pot::<Etp3>(), 50); // Burn: Permill::from_percent(50); **The Burned Etp3 just burned?**
		assert_eq!(Treasury::pot::<Dna>(), 50); // Burn: Permill::from_percent(50); **The Burned Etp3 just burned?**

		// @3: Check balances on the perid after spend perid
		<Treasury as OnInitialize<u64>>::on_initialize(3);
		assert_eq!(Etp3::free_balance(&0), 95); // No changes from last perid
		...
		assert_eq!(Etp3::free_balance(&3), 0); // No changes
		assert_eq!(Dna::free_balance(&3), 0); // No changes
		assert_eq!(Treasury::pot::<Etp3>(), 50); // No changes from last perid
		...
		assert_eq!(Etp3::free_balance(&3), 0); // No changes
		assert_eq!(Dna::free_balance(&3), 0); // No changes
		assert_eq!(Treasury::pot::<Etp3>(), 25); // No changes from last perid
...
fn genesis_funding_works() {
	...
	t.execute_with(|| {
		assert_eq!(Etp3::free_balance(Treasury::account_id()), initial_funding);
		assert_eq!(
			Treasury::pot::<Etp3>(),
			initial_funding - Etp3::minimum_balance()
		);
