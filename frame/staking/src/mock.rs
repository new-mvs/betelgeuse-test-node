...
pub(crate) type Etp3 = betelgeuse_balances::Module<Test, Etp3Instance>;
pub(crate) type Dna = betelgeuse_balances::Module<Test, DnaInstance>;
pub(crate) type Staking = Module<Test>;

pub(crate) type Etp3Error = betelgeuse_balances::Error<Test, Etp3Instance>;
...
impl Trait for Test {
	...
	type Etp3RewardRemainder = Etp3RewardRemainderMock;
	type Etp3Slash = ();
	type Etp3Reward = ();
...
impl ExtBuilder {
	...
	pub fn init_etp3(mut self, init_etp3: bool) -> Self {
		self.init_etp3 = init_etp3;
		self.has_stakers = init_etp3;
		self
	}
	pub fn init_dna(mut self, init_dna: bool) -> Self {
	...
	pub fn build(self) -> sp_io::TestExternalities {
		...
		if self.init_etp3 {
			let _ = betelgeuse_balances::GenesisConfig::<Test, Etp3Instance> {
				balances: vec![
					(1, 10 * balance_factor),
					(2, 20 * balance_factor),
		...
		if self.init_dna {
			let _ = betelgeuse_balances::GenesisConfig::<Test, DnaInstance> {
				balances: vec![
					(1, 10 * balance_factor),
					(2, 20 * balance_factor),
					(3, 300 * balance_factor),
...
pub struct Etp3RewardRemainderMock;
impl OnUnbalanced<Etp3NegativeImbalance<Test>> for Etp3RewardRemainderMock {
	fn on_nonzero_unbalanced(amount: Etp3NegativeImbalance<Test>) {
...
pub fn assert_ledger_consistent(controller: AccountId) {
	...
	assert_eq!(ledger.active_etp3, ledger.etp3_staking_lock.staking_amount);
...
fn bond(stash: AccountId, controller: AccountId, val: StakingBalanceT<Test>) {
	match val {
		StakingBalance::Etp3Balance(r) => {
			let _ = Etp3::make_free_balance_be(&(stash), r);
			let _ = Etp3::make_free_balance_be(&(controller), r);
		}
...
pub(crate) fn current_total_payout_for_duration(era_duration: TsInMs) -> Balance {
	inflation::compute_total_payout::<Test>(
		era_duration,
		Staking::living_time(),
		<Test as Trait>::Cap::get() - Etp3::total_issuance(),
...
pub(crate) fn etp3_balances(who: &AccountId) -> (Balance, Balance) {
	(Etp3::free_balance(who), Etp3::reserved_balance(who))
}
