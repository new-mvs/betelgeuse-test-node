...
mod offchain_election {
	...
	pub fn build_offchain_election_test_ext() {
		for i in (10..=40).step_by(10) {
			...
			// realistic.
			bond_validator(i + 1, i, StakingBalance::Etp3Balance(100));
		}
		...
		bond_nominator(
			voter,
			1000 + voter,
			StakingBalance::Etp3Balance(100),
			vec![11],
		);
		voter = 2;
		bond_nominator(
...
use betelgeuse_support::balance::lock::*;
...
fn force_unstake_works() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// Cant transfer
		assert_noop!(
			Etp3::transfer(Origin::signed(11), 1, 10),
		...
		// Transfer works.
		assert_ok!(Etp3::transfer(Origin::signed(11), 1, 10));
	});
}
...
fn basic_setup_works() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(
			...
			Exposure {
				own_etp3_balance: 1000,
				own_dna_balance: 0,
				own_power: Staking::power_of(&11),
				// Allow error rate 1
				total_power: Staking::power_of(&11)
					+ Staking::currency_to_power(125, Staking::etp3_pool())
					+ 1,
				others: vec![IndividualExposure {
					...
					dna_balance: 0,
					// Allow error rate 1
					power: Staking::currency_to_power(125, Staking::etp3_pool()) + 1,
				}]
			}
		);
		...
		assert_eq!(
			...
			Exposure {
				...
				total_power: Staking::power_of(&21)
					+ Staking::currency_to_power(375, Staking::etp3_pool())
					- 1,
				others: vec![IndividualExposure {
					who: 101,
					// Allow error rate 1
					...
					dna_balance: 0,
					// Allow error rate 1
					power: Staking::currency_to_power(375, Staking::etp3_pool()) - 1,
				}]
			}
		);
		...
		assert_eq!(Etp3::free_balance(10), 1);
		assert_eq!(Etp3::free_balance(10), 1);
...
fn rewards_should_work() {
	...
	ExtBuilder::default().nominate(true).build_and_execute(|| {
		let init_balance_10 = Etp3::free_balance(&10);
		let init_balance_11 = Etp3::free_balance(&11);
		let init_balance_20 = Etp3::free_balance(&20);
		let init_balance_21 = Etp3::free_balance(&21);
		let init_balance_100 = Etp3::free_balance(&100);
		let init_balance_101 = Etp3::free_balance(&101);
		...
		assert_eq!(Etp3::free_balance(&10), init_balance_10);
		assert_eq!(Etp3::free_balance(&11), init_balance_11);
		assert_eq!(Etp3::free_balance(&20), init_balance_20);
		assert_eq!(Etp3::free_balance(&21), init_balance_21);
		assert_eq!(Etp3::free_balance(&100), init_balance_100);
		assert_eq!(Etp3::free_balance(&101), init_balance_101);
		...
		assert_eq_error_rate!(
			Etp3::free_balance(&10),
			init_balance_10 + part_for_10 * total_payout_0 * 2 / 3,
			MICRO,
		);
		assert_eq_error_rate!(Etp3::free_balance(&11), init_balance_11, MICRO);
		assert_eq_error_rate!(
			Etp3::free_balance(&20),
			init_balance_20 + part_for_20 * total_payout_0 * 1 / 3,
			MICRO,
		);
		assert_eq_error_rate!(Etp3::free_balance(&21), init_balance_21, MICRO);
		assert_eq_error_rate!(
			Etp3::free_balance(&100),
			init_balance_100
				+ part_for_100_from_10 * total_payout_0 * 2 / 3
				+ part_for_100_from_20 * total_payout_0 * 1 / 3,
			MICRO,
		);
		assert_eq_error_rate!(Etp3::free_balance(&101), init_balance_101, MICRO);
...
fn staking_should_work() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in 1..5 {
				let _ = Etp3::make_free_balance_be(&i, 2000);
			}
			...
			assert_ok!(Staking::bond(
				Origin::signed(3),
				4,
				StakingBalance::Etp3Balance(1500),
			...
			assert_noop!(Etp3::reserve(&3, 501), Etp3Error::LiquidityRestrictions);
			assert_ok!(Etp3::reserve(&3, 409));
		});
}
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			for i in [1, 2, 3, 4, 5, 10, 11, 20, 21].iter() {
				let _ = Etp3::make_free_balance_be(i, initial_balance);
			}
			...
			assert_ok!(Staking::bond(
				Origin::signed(1),
				2,
				StakingBalance::Etp3Balance(1000),
				RewardDestination::Controller,
				0,
			));
			...
			make_all_reward_payment(0);
			assert_eq!(Etp3::free_balance(&40), 1 + total_payout_0 / 2);
			assert_eq!(Etp3::free_balance(&30), 1 + total_payout_0 / 2);
			...
			assert_eq!(
				...
				Exposure {
					own_etp3_balance: 1000,
					own_dna_balance: 0,
					own_power: Staking::currency_to_power(1000, Staking::etp3_pool()),
					total_power: Staking::currency_to_power(1000, Staking::etp3_pool())
						+ Staking::currency_to_power(800, Staking::etp3_pool()),
					others: vec![
						IndividualExposure {
							who: 3,
							etp3_balance: 400,
							dna_balance: 0,
							power: Staking::currency_to_power(400, Staking::etp3_pool()),
						},
						IndividualExposure {
							who: 1,
							etp3_balance: 400,
			...
			assert_eq!(
				...
				Exposure {
					...
					own_power: Staking::currency_to_power(1000, Staking::etp3_pool()),
					total_power: Staking::currency_to_power(1000, Staking::etp3_pool())
						+ Staking::currency_to_power(1200, Staking::etp3_pool()),
					others: vec![
						IndividualExposure {
							who: 3,
							...
							etp3_balance: 599,
							dna_balance: 0,
							power: Staking::currency_to_power(600, Staking::etp3_pool()),
						},
						IndividualExposure {
							who: 1,
							// expect 600, error rate 1
...
fn nominators_also_get_slashed_pro_rata() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// staked values;
		let nominator_stake = Staking::ledger(100).unwrap().active_etp3;
		let nominator_balance = etp3_balances(&101).0;
		let validator_stake = Staking::ledger(10).unwrap().active_etp3;
		let validator_balance = etp3_balances(&11).0;
		...
		assert!(Staking::ledger(100).unwrap().active_etp3 < nominator_stake);
		assert!(Staking::ledger(10).unwrap().active_etp3 < validator_stake);
		...
		assert_eq_error_rate!(
			Staking::ledger(100).unwrap().active_etp3,
			nominator_stake - nominator_share,
			10,
		);
		assert_eq_error_rate!(
			Staking::ledger(10).unwrap().active_etp3,
			validator_stake - validator_share,
			10,
		);
		assert_eq_error_rate!(
			etp3_balances(&101).0, // free balance
			nominator_balance - nominator_share,
			10,
		);
		assert_eq_error_rate!(
			etp3_balances(&11).0, // free balance
...
fn double_staking_should_fail() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::bond(
			Origin::signed(1),
			2,
			StakingBalance::Etp3Balance(arbitrary_value),
...
fn cannot_transfer_staked_balance() {
	...
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		assert_eq!(Etp3::free_balance(11), 1000);
		// Confirm account 11 (via controller 10) is totally staked
		{
			...
			assert_eq!(
				expo.others
					.iter()
					.fold(expo.own_etp3_balance, |acc, i| acc + i.etp3_balance),
				1000,
			);
		}
		// Confirm account 11 cannot transfer as a result
		assert_noop!(
			Etp3::transfer(Origin::signed(11), 20, 1),
			Etp3Error::LiquidityRestrictions,
		);

		// Give account 11 extra free balance
		let _ = Etp3::make_free_balance_be(&11, 10000);
		// Confirm that account 11 can now transfer some balance
		assert_ok!(Etp3::transfer(Origin::signed(11), 20, 1));
	});
}
...
fn cannot_transfer_staked_balance_2() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Etp3::free_balance(21), 2000);
			...
			assert_noop!(
				Etp3::transfer(Origin::signed(21), 20, 1001),
				Etp3Error::LiquidityRestrictions,
			);
			assert_ok!(Etp3::transfer(Origin::signed(21), 20, 1000));
		});
}
...
fn cannot_reserve_staked_balance() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		// Confirm account 11 (via controller 10) is totally staked
		assert_eq!(
			Staking::eras_stakers(Staking::active_era().unwrap().index, &11).own_etp3_balance,
			1000,
		);
		// Confirm account 11 cannot transfer as a result
		assert_noop!(Etp3::reserve(&11, 1), Etp3Error::LiquidityRestrictions);

		// Give account 11 extra free balance
		let _ = Etp3::make_free_balance_be(&11, 10000);
		// Confirm account 11 can now reserve balance
		assert_ok!(Etp3::reserve(&11, 1));
	});
}
...
fn reward_destination_works() {
	...
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		assert_eq!(Etp3::free_balance(11), 1000 + total_payout_0);
		// Check that amount at stake increased accordingly
		assert_eq!(
			Staking::ledger(&10),
			Some(StakingLedger {
				stash: 11,
				active_etp3: 1000 + total_payout_0,
				etp3_staking_lock: StakingLock {
		...
		assert_eq!(Etp3::free_balance(10), 1 + total_payout_2);
		// Check that amount at stake is NOT increased
		assert_eq!(
			Staking::ledger(&10),
			Some(StakingLedger {
		...
		assert_eq!(Etp3::free_balance(11), recorded_stash_balance);
	});
}
...
fn validator_payment_prefs_work() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		let balance_era_1_10 = Etp3::free_balance(&10);
		let balance_era_1_100 = Etp3::free_balance(&100);
...
fn bond_extra_works() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		let _ = Etp3::make_free_balance_be(&11, 1000000);
		...
		assert_ok!(Staking::bond_extra(
			Origin::signed(11),
			StakingBalance::Etp3Balance(Balance::max_value()),
			0,
		));
...
fn too_many_unbond_calls_should_not_work() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// can't do more.
		assert_noop!(
			Staking::unbond(Origin::signed(10), StakingBalance::Etp3Balance(1)),
...
fn rebond_works() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			Staking::unbond(Origin::signed(10), StakingBalance::Etp3Balance(900)).unwrap();
			assert_eq!(
				Staking::ledger(&10),
				Some(StakingLedger {
					stash: 11,
			...
			Staking::unbond(Origin::signed(10), StakingBalance::Etp3Balance(300)).unwrap();
			Staking::unbond(Origin::signed(10), StakingBalance::Etp3Balance(300)).unwrap();
			Staking::unbond(Origin::signed(10), StakingBalance::Etp3Balance(300)).unwrap();
			assert_eq!(
				Staking::ledger(&10),
...
fn rebond_is_fifo() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			Staking::unbond(Origin::signed(10), StakingBalance::Etp3Balance(400)).unwrap();
			assert_eq!(
				Staking::ledger(&10),
				Some(StakingLedger {
					stash: 11,
			...
			Staking::unbond(Origin::signed(10), StakingBalance::Etp3Balance(200)).unwrap();
			assert_eq!(
				Staking::ledger(&10),
				Some(StakingLedger {
					stash: 11,
		...
		.build_and_execute(|| {
			...
			}
			{
				...
				let total_etp3 = expo
					...
					.iter()
					.fold(expo.own_etp3_balance, |acc, i| acc + i.etp3_balance);
				assert_eq!(total_etp3, 2000);
			}

			// Give the man some money.
			let _ = Etp3::make_free_balance_be(&10, 1000);
			let _ = Etp3::make_free_balance_be(&20, 1000);
			...
			<ErasStakers<Test>>::insert(
				...
				Exposure {
					own_etp3_balance: 69,
					own_dna_balance: 0,
					total_power: Staking::currency_to_power(69, Staking::etp3_pool()),
					own_power: Staking::currency_to_power(69, Staking::etp3_pool()),
					others: vec![],
				},
			);
			...
			let _11_balance = Etp3::free_balance(&11);
			...
			// -- new infos
			{
				...
				let total_etp3 = expo
					...
					.iter()
					.fold(expo.own_etp3_balance, |acc, i| acc + i.etp3_balance);
				assert_eq!(total_etp3, 1000 + total_payout_0 / 2);
			}
			{
				let expo = Staking::eras_stakers(Staking::active_era().unwrap().index, 21);
				let total_etp3 = expo
					...
					.iter()
					.fold(expo.own_etp3_balance, |acc, i| acc + i.etp3_balance);
				assert_eq!(total_etp3, 69 + total_payout_0 / 2);
			}
		});
}
...
fn on_free_balance_zero_stash_removes_validator() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			// Check the balance of the validator account
			assert_eq!(Etp3::free_balance(10), 256);
			// Check the balance of the stash account
			assert_eq!(Etp3::free_balance(11), 256000);
			...
			let _ = Etp3::slash(&10, Balance::max_value());
			...
			let _ = Etp3::slash(&11, Balance::max_value());
			// Check total balance of stash
			assert_eq!(Etp3::free_balance(&11), 0);
...
fn on_free_balance_zero_stash_removes_nominator() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Etp3::free_balance(&10), 0);
...
fn switching_roles() {
	...
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		// put some money in account that we'll use.
		for i in 1..7 {
			let _ = Etp3::deposit_creating(&i, 5000);
		}
		...
		assert_ok!(Staking::bond(
			Origin::signed(1),
			2,
			StakingBalance::Etp3Balance(2000),
			RewardDestination::Controller,
			0,
		));
		...
		assert_ok!(Staking::bond(
			Origin::signed(3),
			4,
			StakingBalance::Etp3Balance(500),
			RewardDestination::Controller,
			0,
		));
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Staking::bond(
				Origin::signed(1),
				2,
				StakingBalance::Etp3Balance(5),
				RewardDestination::Controller,
				0,
			));
			assert_eq!(
				Etp3::locks(&1)[0].locked_amount(Some(System::block_number())),
				5
			);
...
fn bond_with_little_staked_value_bounded() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let init_balance_2 = Etp3::free_balance(&2);
			let init_balance_10 = Etp3::free_balance(&10);
			...
			assert_eq!(Etp3::free_balance(&2), init_balance_2);
...
fn bond_with_duplicate_vote_should_be_ignored_by_npos_election() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_ok!(Staking::bond_extra(
				Origin::signed(31),
				StakingBalance::Etp3Balance(999),
				0
			));

			assert_eq!(
				<Validators<Test>>::iter()
					.map(|(v, _)| (v, Staking::ledger(v - 1).unwrap().active_etp3))
					.collect::<Vec<_>>(),
...
fn phragmen_should_not_overflow() {
	ExtBuilder::default().nominate(false).build_and_execute(|| {
		...
		bond_validator(3, 2, StakingBalance::Etp3Balance(CAP));
		bond_validator(5, 4, StakingBalance::DnaBalance(Balance::max_value() - 1));

		bond_nominator(7, 6, StakingBalance::Etp3Balance(1), vec![3, 5]);
...
fn reward_validator_slashing_validator_does_not_overflow() {
	ExtBuilder::default().build_and_execute(|| {
		...
		// Set staker
		let _ = Etp3::make_free_balance_be(&11, stake);

		let exposure = Exposure::<AccountId, Balance, Balance> {
			own_etp3_balance: stake,
			own_dna_balance: 0,
			own_power: Staking::currency_to_power(stake, Staking::etp3_pool()),
			total_power: Staking::currency_to_power(stake, Staking::etp3_pool()),
			others: vec![],
		};
		...
		let _ = Etp3::deposit_creating(&Staking::account_id(), stake);
		assert_ok!(Staking::payout_stakers(Origin::signed(1337), 11, 0));
		assert_eq!(Etp3::free_balance(&11), stake * 2);

		// Set staker
		let _ = Etp3::make_free_balance_be(&11, stake);
		let _ = Etp3::make_free_balance_be(&2, stake);
		...
		Staking::bond(
			Origin::signed(2),
			20000,
			StakingBalance::Etp3Balance(stake - 1),
			RewardDestination::default(),
			0,
		)
		.unwrap();
		...
		<ErasStakers<Test>>::insert(
			...
			Exposure {
				...
				total_power: Staking::currency_to_power(stake, Staking::etp3_pool()),
				own_dna_balance: 0,
				own_power: Staking::currency_to_power(1, Staking::etp3_pool()),
				others: vec![IndividualExposure {
					who: 2,
					etp3_balance: stake - 1,
					dna_balance: 0,
					power: Staking::currency_to_power(stake - 1, Staking::etp3_pool()),
				}],
			},
		);
		...
		assert_eq_error_rate!(Etp3::free_balance(&11), stake - 1000, MICRO);
...
fn unbonded_balance_is_not_slashable() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_ok!(Staking::unbond(
			Origin::signed(10),
			StakingBalance::Etp3Balance(800)
		));
...
#[test]
fn offence_ensures_new_era_without_clobbeetp3() {
	ExtBuilder::default().build_and_execute(|| {
...
fn slashing_performed_according_exposure() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		on_offence_now(
			&[OffenceDetails {
				offender: (
					11,
					Exposure {
						total_power: Staking::currency_to_power(500, Staking::etp3_pool()),
						own_etp3_balance: 500,
						own_dna_balance: 0,
						own_power: 0,
						others: vec![],
					},
		...
		assert_eq!(Etp3::free_balance(11), 1000 - 250);
	});
}
	...
	ExtBuilder::default().build_and_execute(|| {
...

		{
			...
			let total_etp3 = expo
				...
				.iter()
				.fold(expo.own_etp3_balance, |acc, i| acc + i.etp3_balance);
			assert_eq!(total_etp3, initial_balance);
		}
		...
		assert_eq!(Etp3::free_balance(&1), 10 + reward_each);
		assert_eq!(Etp3::free_balance(&2), 20 + reward_each);
	});
}
...
fn subsequent_reports_in_same_span_pay_out_less() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Etp3::free_balance(&1), 10 + reward);
		...
		assert_eq!(Etp3::free_balance(&1), 10 + prior_payout + reward);
	});
}
...
fn invulnerables_are_not_slashed() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Etp3::free_balance(&21), 2000 - (2 * initial_balance / 10));

			// ensure that nominators were slashed as well.
			for (initial_balance, other) in nominator_balances.into_iter().zip(exposure.others) {
				assert_eq_error_rate!(
					Etp3::free_balance(&other.who),
					initial_balance - (2 * other.etp3_balance / 10),
					1
				);
			}
		});
...
fn only_slash_for_max_in_era() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Etp3::free_balance(11), 500);
		assert_eq!(Staking::force_era(), Forcing::ForceNew);
		...
		assert_eq!(Etp3::free_balance(11), 400);
	})
}
...
fn garbage_collection_after_slashing() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			assert_eq!(Etp3::free_balance(11), 256_000);
			...
			assert_eq!(Etp3::free_balance(11), 256_000 - 25_600);
			...
			assert_eq!(Etp3::free_balance(11), 0);
			assert_eq!(Etp3::total_balance(&11), 0);
...
fn garbage_collection_on_window_pruning() {
	...
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Etp3::free_balance(101), 2000);
		...
		assert_eq!(Etp3::free_balance(11), 900);
		assert_eq!(Etp3::free_balance(101), 2000 - (nominated_value / 10));
...
fn slashing_nominators_by_span_max() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Etp3::free_balance(101), 2000 - slash_1_amount);
		...
		assert_eq!(Etp3::free_balance(11), 900);
		assert_eq!(Etp3::free_balance(21), 1700);

		let slash_2_amount = Perbill::from_percent(30) * nominated_value_21;
		assert!(slash_2_amount > slash_1_amount);

		// only the maximum slash in a single span is taken.
		assert_eq!(Etp3::free_balance(101), 2000 - slash_2_amount);
		...
		assert_eq!(Etp3::free_balance(11), 800);
		assert_eq!(Etp3::free_balance(21), 1700);
...
fn slashes_are_summed_across_spans() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq!(Etp3::free_balance(21), 1900);
		...
		assert_eq!(get_span(21).iter().collect::<Vec<_>>(), expected_spans);
		assert_eq!(Etp3::free_balance(21), 1810);
	});
}
...
fn remove_deferred() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Etp3::free_balance(11), 950);
			assert_eq!(Etp3::free_balance(101), 2000 - actual_slash);
		})
}
...
fn slash_kicks_validators_not_nominators_and_disables_nominator_for_kicked_validator() {
	ExtBuilder::default().build_and_execute(|| {
		...
		assert_eq_error_rate!(
			...
			Staking::currency_to_power(1000 + 125, Staking::etp3_pool()),
			1
		);
		assert_eq_error_rate!(
			exposure_21.total_power,
			Staking::currency_to_power(1000 + 375, Staking::etp3_pool()),
			1
		);
		...
		assert_eq!(Etp3::free_balance(11), 900);
		assert_eq!(Etp3::free_balance(101), 2000 - nominator_slash_amount_11);
		...
		assert_eq!(
			exposure_11.total_power,
			Staking::currency_to_power(900, Staking::etp3_pool())
		);
...
fn claim_reward_at_the_last_era_and_no_double_claim_and_invalid_claim() {
	...
	ExtBuilder::default().nominate(true).build_and_execute(|| {
		...
		let _ = Etp3::deposit_creating(&999, 1_000_000_000);
		// Compute total payout now for whole duration as other parameter won't change
		let total_payout_1 = current_total_payout_for_duration(3000);
		assert!(total_payout_1 > 10); // Test is meaningful if reward something
...
fn test_max_nominator_rewarded_per_validator_and_cant_steal_someone_else_reward() {
	...
	ExtBuilder::default().build_and_execute(|| {
		for i in 0..=<Test as Trait>::MaxNominatorRewardedPerValidator::get() {
			...
			Etp3::make_free_balance_be(&stash, balance);
			assert_ok!(Staking::bond(
				Origin::signed(stash),
				controller,
				StakingBalance::Etp3Balance(balance),
				RewardDestination::Stash,
				0,
			));
		...
		for i in 0..=<Test as Trait>::MaxNominatorRewardedPerValidator::get() {
			...
			let balance = 10_000 + i as Balance;
			if stash == 10_000 {
				assert!(Etp3::free_balance(&stash) == balance);
			} else {
				assert!(Etp3::free_balance(&stash) > balance);
			}
		}
...
fn test_payout_stakers() {
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			let balance = 1000;
			// Create a validators:
			bond_validator(11, 10, StakingBalance::Etp3Balance(balance)); // Default(64)
			...
			for i in 0..100 {
				bond_nominator(
					1000 + i,
					100 + i,
					StakingBalance::Etp3Balance(balance + i as Balance),
					vec![11],
				);
			}
			...
			assert!(Etp3::free_balance(&10) > balance);
			for i in 36..100 {
				assert!(Etp3::free_balance(&(100 + i)) > balance + i as Balance);
			}
			// The bottom 36 do not
			for i in 0..36 {
				assert_eq!(Etp3::free_balance(&(100 + i)), balance + i as Balance);
			}
...
#[test]
fn bond_duetp3_era_correctly_populates_claimed_rewards() {
	ExtBuilder::default()
		.has_stakers(false)
		.build_and_execute(|| {
			// Era = None
			bond_validator(9, 8, StakingBalance::Etp3Balance(1000));
			assert_eq!(
				Staking::ledger(&8),
				Some(StakingLedger {
					stash: 9,
			...
			start_era(5);
			bond_validator(11, 10, StakingBalance::Etp3Balance(1000));
			assert_eq!(
				Staking::ledger(&10),
				Some(StakingLedger {
					stash: 11,
			...
			start_era(99);
			bond_validator(13, 12, StakingBalance::Etp3Balance(1000));
			assert_eq!(
				Staking::ledger(&12),
				Some(StakingLedger {
					stash: 13,
...
fn offences_weight_calculated_correctly() {
	ExtBuilder::default().nominate(true).build_and_execute(|| {
		...
		let n_offence_unapplied_weight = <Test as frame_system::Trait>::DbWeight::get()
			.reads_writes(4, 1)
		...
		> = (1..10)
			.map(|i| OffenceDetails {
				offender: (
					i,
					Staking::eras_stakers(Staking::active_era().unwrap().index, i),
				),
				reporters: vec![],
			})
			.collect();
...
fn payout_creates_controller() {
	ExtBuilder::default()
		.has_stakers(false)
		.build_and_execute(|| {
			let balance = StakingBalance::Etp3Balance(1000);
			// Create a validator:
			bond_validator(11, 10, balance);

			// Create a stash/controller pair
			bond_nominator(1234, 1337, StakingBalance::Etp3Balance(100), vec![11]);

			// kill controller
			assert_ok!(Etp3::transfer(Origin::signed(1337), 1234, 100));
			assert_eq!(Etp3::free_balance(1337), 0);
			...
			assert!(Etp3::free_balance(1337) > 0);
		})
}
...
fn payout_to_any_account_works() {
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			assert_eq!(Etp3::free_balance(42), 0);
