...
//! <!-- Original author of paragraph: @gavofyork -->
//!
//! The staking system in Betelgeuse NPoS is designed to make the following possible:
//!
...
//! #### Validating
//!
//! A **validator** takes the role of either validating blocks or ensuetp3 their finality,
...
//! use betelgeuse_staking as staking;
//!
...
//! they received duetp3 the era. Points are added to a validator using
...
// TODO: offchain phragmen test https://github.com/betelgeuse-network/betelgeuse-common/issues/97
...
mod types {
	...
	pub type Etp3PositiveImbalance<T> =
		<Etp3Currency<T> as Currency<AccountId<T>>>::PositiveImbalance;
	pub type Etp3NegativeImbalance<T> =
		<Etp3Currency<T> as Currency<AccountId<T>>>::NegativeImbalance;
	...
	pub type StakingLedgerT<T> =
		StakingLedger<AccountId<T>, Etp3Balance<T>, DnaBalance<T>, BlockNumber<T>>;
	pub type StakingBalanceT<T> = StakingBalance<Etp3Balance<T>, DnaBalance<T>>;

	pub type ExposureT<T> = Exposure<AccountId<T>, Etp3Balance<T>, DnaBalance<T>>;
	pub type ElectionResultT<T> = ElectionResult<AccountId<T>, Etp3Balance<T>, DnaBalance<T>>;
...
// --- betelgeuse ---
use betelgeuse_staking_rpc_runtime_api::RuntimeDispatchInfo;
use betelgeuse_support::{
	balance::lock::*,
	impl_rpc,
...
pub trait Trait: frame_system::Trait + SendTransactionTypes<Call<Self>> {
	...
	/// The *RING* currency.
	type Etp3Currency: LockableCurrency<Self::AccountId, Moment = Self::BlockNumber>;
	/// Tokens have been minted and are unused for validator-reward.
	/// See [Era payout](./index.html#era-payout).
	type Etp3RewardRemainder: OnUnbalanced<Etp3NegativeImbalance<Self>>;
	/// Handler for the unbalanced *RING* reduction when slashing a staker.
	type Etp3Slash: OnUnbalanced<Etp3NegativeImbalance<Self>>;
	/// Handler for the unbalanced *RING* increment when rewarding a staker.
	type Etp3Reward: OnUnbalanced<Etp3PositiveImbalance<Self>>;
	...
	/// Betelgeuse's hard cap default 10_000_000_000 * 10^9
	type Cap: Get<Etp3Balance<Self>>;
	/// Betelgeuse's staking vote default 1_000_000_000
...
where
	...
	T: pallet_session::historical::Trait<
		FullIdentification = Exposure<AccountId<T>, Etp3Balance<T>, DnaBalance<T>>,
...
decl_storage! {
	trait Store for Module<T: Trait> as BetelgeuseStaking {
		...
		pub ErasValidatorReward
			...
			: map hasher(twox_64_concat) EraIndex => Option<Etp3Balance<T>>;
		...
		pub UnappliedSlashes
			...
			=> Vec<UnappliedSlash<T::AccountId, Etp3Balance<T>, DnaBalance<T>>>;
		...
		SpanSlash
			...
			=> slashing::SpanRecord<Etp3Balance<T>, DnaBalance<T>>;
		...
		/// use for calculate betelgeuse era payout
		...
		/// Total *RING* in pool.
		pub Etp3Pool get(fn etp3_pool): Etp3Balance<T>;
		/// Total *KTON* in pool.
	...
	add_extra_genesis {
		...
		build(|config: &GenesisConfig<T>| {
			for &(ref stash, ref controller, etp3_to_be_bonded, ref status) in &config.stakers {
				assert!(
					T::Etp3Currency::free_balance(&stash) >= etp3_to_be_bonded,
					"Stash does not have enough balance to bond.",
				);
				let _ = <Module<T>>::bond(
					T::Origin::from(Some(stash.to_owned()).into()),
					T::Lookup::unlookup(controller.to_owned()),
					StakingBalance::Etp3Balance(etp3_to_be_bonded),
...
decl_event!(
		...
		DnaBalance = DnaBalance<T>,
	{
		...
		EraPayout(EraIndex, Etp3Balance, Etp3Balance),

		/// The staker has been rewarded by this amount. [stash, amount]
		Reward(AccountId, Etp3Balance),

		/// One validator (and its nominators) has been slashed by the given amount.
		/// [validator, amount, amount]
		Slash(AccountId, Etp3Balance, DnaBalance),
		...
		BondEtp3(Etp3Balance, TsInMs, TsInMs),
		/// An account has bonded this amount. [amount, start, end]
		///
		/// NOTE: This event is only emitted when funds are bonded via a dispatchable. Notably,
		/// it will not be emitted for staking rewards when they are added to stake.
		...
		UnbondEtp3(Etp3Balance, BlockNumber),
		/// An account has unbonded this amount. [amount, now]
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		/// Betelgeuse's hard cap default 10_000_000_000 * 10^9
		const Cap: Etp3Balance<T> = T::Cap::get();
		...
		fn bond(
			...
			match value {
				StakingBalance::Etp3Balance(value) => {
					// reject a bond which is considered to be _dust_.
					ensure!(
						value >= T::Etp3Currency::minimum_balance(),
			...
			match value {
				StakingBalance::Etp3Balance(value) => {
					let stash_balance = T::Etp3Currency::free_balance(&stash);
					let value = value.min(stash_balance);
					let promise_month = promise_month.min(36);
					let (start_time, expire_time) = Self::bond_etp3(
						&stash,
						&controller,
						value,
						promise_month,
					...
					Self::deposit_event(RawEvent::BondEtp3(value, start_time, expire_time));
				},
				StakingBalance::DnaBalance(value) => {
		...
		fn bond_extra(origin, max_additional: StakingBalanceT<T>, promise_month: u8) {
			...
			match max_additional {
				 StakingBalance::Etp3Balance(max_additional) => {
					let stash_balance = T::Etp3Currency::free_balance(&stash);

					if let Some(extra) = stash_balance.checked_sub(
						&ledger.etp3_locked_amount_at(<frame_system::Module<T>>::block_number())
					) {
						...
						Self::deposit_event(RawEvent::BondEtp3(extra, start_time, expire_time));
					}
				},
		...
		/// Deposit some extra amount etp3, and return dna to the controller.
		///
		...
		fn deposit_extra(origin, value: Etp3Balance<T>, promise_month: u8) {
			let stash = ensure_signed(origin)?;
			let controller = Self::bonded(&stash).ok_or(<Error<T>>::NotStash)?;
			let ledger = Self::ledger(&controller).ok_or(<Error<T>>::NotController)?;
			...
			let value = value.min(active_etp3.saturating_sub(*active_deposit_etp3));
			...
			*active_deposit_etp3 = active_deposit_etp3.saturating_add(value);
		...
		///
		/// After all pledged Etp3 and Dna are unbonded, the bonded accounts, namely stash and
		/// controller, will also be unbonded.  Once user want to bond again, the `bond` method
		/// should be called. If there are still pledged Etp3 or Dna and user want to bond more
		...
		fn unbond(origin, value: StakingBalanceT<T>) {
			...
			//	 2. `let c_ = a as u32 + b as u32; c_ < c`
				...
				(etp3_staking_lock.unbondings.len() + dna_staking_lock.unbondings.len())
					< MAX_UNLOCKING_CHUNKS,
				<Error<T>>::NoMoreChunks,
			);

			let mut unbond_etp3: Etp3Balance<T> = Zero::zero();
			let mut unbond_dna: DnaBalance<T> = Zero::zero();

			match value {
				StakingBalance::Etp3Balance(r) => {
					// Only active normal etp3 can be unbond:
					// `active_etp3 = active_normal_etp3 + active_deposit_etp3`
					let active_normal_etp3 = *active_etp3 - *active_deposit_etp3;
					unbond_etp3 = r.min(active_normal_etp3);

					if !unbond_etp3.is_zero() {
						*active_etp3 -= unbond_etp3;

						// Avoid there being a dust balance left in the staking system.
						if (*active_etp3 < T::Etp3Currency::minimum_balance())
							&& (*active_dna < T::DnaCurrency::minimum_balance()) {
							unbond_etp3 += *active_etp3;
							unbond_dna += *active_dna;

							*active_etp3 = Zero::zero();
							*active_dna = Zero::zero();
						}

						etp3_staking_lock.unbondings.push(Unbonding {
							amount: unbond_etp3,
							until: now + T::BondingDurationInBlockNumber::get(),
						});

						Self::deposit_event(RawEvent::UnbondEtp3(unbond_etp3, now));
					...
					if !unbond_dna.is_zero() {
						*active_dna -= unbond_dna;
						...
						if (*active_dna < T::DnaCurrency::minimum_balance())
							&& (*active_etp3 < T::Etp3Currency::minimum_balance()) {
							unbond_dna += *active_dna;
							unbond_etp3 += *active_etp3;
			...
			// TODO: https://github.com/betelgeuse-network/betelgeuse-common/issues/96
			// FIXME: https://github.com/betelgeuse-network/betelgeuse-common/issues/121
			// let StakingLedger {
			// 	active_etp3,
			// 	active_dna,
			// 	..
			...
			// if active_etp3.is_zero() && active_dna.is_zero() {
			// 	//
			...
			// 	// blocks.
			// 	//
			// 	// T::Etp3::remove_lock(STAKING_ID, &stash);
			// 	// T::Dna::remove_lock(STAKING_ID, &stash);
			// 	// Self::kill_stash(&stash)?;
			// }
		}

		/// Stash accounts can get their etp3 back after the depositing time exceeded,
		/// and the etp3 getting back is still in staking status.
		///
		/// # <weight>
		/// - Independent of the arguments. Insignificant complexity.
		/// - One storage read.
		...
		/// Claim deposits while the depositing time has not been exceeded, the etp3
		/// will not be slashed, but the account is required to pay KTON as punish.
		///
		/// Refer to https://talk.betelgeuse.network/topics/55
		///
...

			{
				...
				deposit_items.retain(|item| {
					...
					if T::DnaCurrency::usable_balance(stash) >= dna_slash {
						*active_deposit_etp3 = active_deposit_etp3.saturating_sub(item.value);
		...
		fn force_unstake(origin, stash: T::AccountId, num_slashing_spans: u32) {
			...
			// remove the lock.
			T::Etp3Currency::remove_lock(STAKING_ID, &stash);
		...
		fn rebond(
			origin,
			#[compact] plan_to_rebond_etp3: Etp3Balance<T>,
			...
			ledger.etp3_staking_lock.update(now);
			ledger.dna_staking_lock.update(now);

			ensure!(
				!ledger.etp3_staking_lock.unbondings.is_empty()
					|| !ledger.dna_staking_lock.unbondings.is_empty(),
				<Error<T>>::NoUnlockChunk
			);

			let origin_active_etp3 = ledger.active_etp3;
			let origin_active_dna = ledger.active_dna;

			ledger.rebond(plan_to_rebond_etp3, plan_to_rebond_dna);

			Self::update_ledger(&controller, &mut ledger);

			let rebond_etp3 = ledger.active_etp3.saturating_sub(origin_active_etp3);
			let rebond_dna = ledger.active_dna.saturating_sub(origin_active_dna);

			if !rebond_etp3.is_zero() {
				let now = T::UnixTime::now().as_millis().saturated_into::<TsInMs>();

				Self::deposit_event(RawEvent::BondEtp3(rebond_etp3, now, now));
			}
			...
			Ok(Some(
				35 * WEIGHT_PER_MICROS
					+ 50 * WEIGHT_PER_NANOS * (
						ledger.etp3_staking_lock.unbondings.len() as Weight
		...
		///    This should report all the storage items that will be deleted by cleaetp3 old
		...
		fn reap_stash(_origin, stash: T::AccountId, num_slashing_spans: u32) {
			ensure!(T::Etp3Currency::total_balance(&stash).is_zero(), <Error<T>>::FundedTarget);
		...
		/// solution to be rejected. These two storage items are set duetp3 the election window and
...
impl<T: Trait> Module<T> {
	...
	/// Update the ledger while bonding etp3 and compute the *KTON* reward
	fn bond_etp3(
		stash: &T::AccountId,
		...
		*active_etp3 = active_etp3.saturating_add(value);
	...
	// For *RING* power = etp3_ratio * POWER_COUNT / 2
	// For *KTON* power = dna_ratio * POWER_COUNT / 2
	...
	pub fn power_of(stash: &T::AccountId) -> Power {
		...
		Self::bonded(stash)
			...
			.map(|l| {
				Self::currency_to_power::<_>(l.active_etp3, Self::etp3_pool())
	...
	pub fn stake_of(stash: &T::AccountId) -> (Etp3Balance<T>, DnaBalance<T>) {
		// Weight note: consider making the stake accessible through stash.
		Self::bonded(stash)
			.and_then(Self::ledger)
			.map(|l| (l.active_etp3, l.active_dna))
			.unwrap_or_default()
	}
	...
	fn do_payout_stakers(validator_stash: T::AccountId, era: EraIndex) -> DispatchResult {
		...
		let module_account = Self::account_id();
			...
			T::Etp3Currency::usable_balance(&module_account) >= validator_total_payout,
			<Error<T>>::PayoutIns
		);
		...
		let mut actual_payout = <Etp3PositiveImbalance<T>>::zero();
		...
		for nominator in exposure.others.iter() {
			...
			let nominator_reward: Etp3Balance<T> =
	...
	fn update_ledger(controller: &T::AccountId, ledger: &mut StakingLedgerT<T>) {
		...
		if *active_etp3 != etp3_staking_lock.staking_amount {
			let origin_active_etp3 = etp3_staking_lock.staking_amount;

			etp3_staking_lock.staking_amount = *active_etp3;

			<Etp3Pool<T>>::mutate(|pool| {
				if origin_active_etp3 > *active_etp3 {
					*pool = pool.saturating_sub(origin_active_etp3 - *active_etp3);
				} else {
					*pool = pool.saturating_add(*active_etp3 - origin_active_etp3);
				}
			});

			T::Etp3Currency::set_lock(
				STAKING_ID,
				&ledger.stash,
				LockFor::Staking(ledger.etp3_staking_lock.clone()),
				WithdrawReasons::all(),
			);
		}
	...
	fn make_payout(
		stash: &T::AccountId,
		amount: Etp3Balance<T>,
	) -> Option<Etp3PositiveImbalance<T>> {
		let dest = Self::payee(stash);
		match dest {
			RewardDestination::Controller => Self::bonded(stash).and_then(|controller| {
				Some(T::Etp3Currency::deposit_creating(&controller, amount))
			}),
			RewardDestination::Stash => T::Etp3Currency::deposit_into_existing(stash, amount).ok(),
			RewardDestination::Staked => Self::bonded(stash)
				.and_then(|c| Self::ledger(&c).map(|l| (c, l)))
				.and_then(|(c, mut l)| {
					let r = T::Etp3Currency::deposit_into_existing(stash, amount).ok();

					if r.is_some() {
						l.active_etp3 += amount;
			...
			RewardDestination::Account(dest_account) => {
				Some(T::Etp3Currency::deposit_creating(&dest_account, amount))
			}
		}
	}
	...
	fn end_era(active_era: ActiveEraInfo, _session_index: SessionIndex) {
		// Note: active_era_start can be None if end era is called duetp3 genesis config.
		if let Some(active_era_start) = active_era.start {
			...
			let (validator_payout, max_payout) = inflation::compute_total_payout::<T>(
				era_duration,
				Self::living_time(),
				T::Cap::get().saturating_sub(T::Etp3Currency::total_issuance()),
				PayoutFraction::get(),
			);
			...
			T::Etp3Currency::deposit_creating(&Self::account_id(), validator_payout);
			T::Etp3RewardRemainder::on_unbalanced(T::Etp3Currency::issue(rest));
		}
	}
	...
	fn collect_exposure(supports: SupportMap<T::AccountId>) -> Vec<(T::AccountId, ExposureT<T>)> {
		supports
			...
			.map(|(validator, support)| {
				...
				let mut own_etp3_balance: Etp3Balance<T> = Zero::zero();
				...
				support.voters.into_iter().for_each(|(nominator, weight)| {
					...
					let (origin_etp3_balance, origin_dna_balance) = Self::stake_of(&nominator);

					let etp3_balance = if let Ok(etp3_balance) = multiply_by_rational(
						origin_etp3_balance.saturated_into(),
						weight,
						origin_weight,
					) {
						etp3_balance.saturated_into()
					} else {
						log!(
							error,
					...
					if nominator == validator {
						own_etp3_balance = own_etp3_balance.saturating_add(etp3_balance);
...
impl<T: Trait> OnDepositRedeem<T::AccountId, Etp3Balance<T>> for Module<T> {
	fn on_deposit_redeem(
		...
		// The timestamp unit is different between Ethereum and Betelgeuse
		...
		if let Some(controller) = Self::bonded(&stash) {
			...
			T::Etp3Currency::transfer(&backing, &stash, amount, KeepAlive)?;
			...
			*active_etp3 = active_etp3.saturating_add(amount);
			*active_deposit_etp3 = active_deposit_etp3.saturating_add(amount);
			deposit_items.push(TimeDepositItem {
				value: amount,
				start_time,
				expire_time,
		...
		Self::deposit_event(RawEvent::BondEtp3(amount, start_time, expire_time));
...
#[derive(PartialEq, Eq, Clone, Encode, Decode, RuntimeDebug)]
pub enum StakingBalance<Etp3Balance, DnaBalance>
where
	Etp3Balance: HasCompact,
	DnaBalance: HasCompact,
{
	Etp3Balance(Etp3Balance),
	DnaBalance(DnaBalance),
}
impl<Etp3Balance, DnaBalance> Default for StakingBalance<Etp3Balance, DnaBalance>
where
	Etp3Balance: Zero + HasCompact,
	DnaBalance: Zero + HasCompact,
{
	fn default() -> Self {
		StakingBalance::Etp3Balance(Zero::zero())
	}
}
...
pub struct StakingLedger<AccountId, Etp3Balance, DnaBalance, BlockNumber>
where
	Etp3Balance: HasCompact,
	DnaBalance: HasCompact,
{
	...
	pub deposit_items: Vec<TimeDepositItem<Etp3Balance>>,

	/// The staking lock on *RING* balance, use for updating betelgeuse balance module's lock
	pub etp3_staking_lock: StakingLock<Etp3Balance, BlockNumber>,
	/// The staking lock on *KTON* balance, use for updating betelgeuse balance module's lock
...
impl<AccountId, Etp3Balance, DnaBalance, BlockNumber>
	StakingLedger<AccountId, Etp3Balance, DnaBalance, BlockNumber>
where
	Etp3Balance: Copy + AtLeast32BitUnsigned + Saturating,
	DnaBalance: Copy + AtLeast32BitUnsigned + Saturating,
	BlockNumber: Copy + PartialOrd,
	TsInMs: PartialOrd,
{
	pub fn etp3_locked_amount_at(&self, at: BlockNumber) -> Etp3Balance {
		self.etp3_staking_lock.locked_amount(at)
	}
	...
	fn rebond(&mut self, plan_to_rebond_etp3: Etp3Balance, plan_to_rebond_dna: DnaBalance) {
		...
		update(
			&mut self.active_etp3,
			&mut self.etp3_staking_lock,
			plan_to_rebond_etp3,
		);
		update(
			&mut self.active_dna,
	...
	pub fn slash(
		&mut self,
		slash_etp3: Etp3Balance,
		slash_dna: DnaBalance,
		bn: BlockNumber,
		ts: TsInMs,
	) -> (Etp3Balance, DnaBalance) {
		let slash_out_of = |active_etp3: &mut Etp3Balance,
		                    active_deposit_etp3: &mut Etp3Balance,
		                    deposit_item: &mut Vec<TimeDepositItem<Etp3Balance>>,
		                    active_dna: &mut DnaBalance,
		                    slash_etp3: &mut Etp3Balance,
		                    slash_dna: &mut DnaBalance| {
			let slashable_active_etp3 = (*slash_etp3).min(*active_etp3);
			let slashable_active_dna = (*slash_dna).min(*active_dna);

			if !slashable_active_etp3.is_zero() {
				let slashable_normal_etp3 = *active_etp3 - *active_deposit_etp3;
				if let Some(mut slashable_deposit_etp3) =
					slashable_active_etp3.checked_sub(&slashable_normal_etp3)
				{
					*active_deposit_etp3 -= slashable_deposit_etp3;

					deposit_item.drain_filter(|item| {
						if ts >= item.expire_time {
							true
						} else {
							if slashable_deposit_etp3.is_zero() {
								false
							} else {
								if let Some(new_slashable_deposit_etp3) =
									slashable_deposit_etp3.checked_sub(&item.value)
								{
									slashable_deposit_etp3 = new_slashable_deposit_etp3;
									true
								} else {
									item.value -= sp_std::mem::replace(
										&mut slashable_deposit_etp3,
										Zero::zero(),
									);
									false
								}
				...
				*active_etp3 -= slashable_active_etp3;
				*slash_etp3 -= slashable_active_etp3;
			}
		...
		let (mut apply_slash_etp3, mut apply_slash_dna) = (slash_etp3, slash_dna);
		let StakingLedger {
			active_etp3,
			active_deposit_etp3,
			deposit_items,
		...
		slash_out_of(
			...
			deposit_items,
			active_dna,
			&mut apply_slash_etp3,
			&mut apply_slash_dna,
		);

		if !apply_slash_etp3.is_zero() {
			etp3_staking_lock.unbondings.drain_filter(|lock| {
				if bn >= lock.until {
					true
				} else {
					if apply_slash_etp3.is_zero() {
						...
						if apply_slash_etp3 >= lock.amount {
							apply_slash_etp3 -= lock.amount;
							true
						} else {
							lock.amount -=
								sp_std::mem::replace(&mut apply_slash_etp3, Zero::zero());
							false
						}
					}
				}
		...
		(slash_etp3 - apply_slash_etp3, slash_dna - apply_slash_dna)
	}
}

/// The *RING* under deposit.
#[derive(PartialEq, Eq, Clone, Encode, Decode, RuntimeDebug)]
pub struct TimeDepositItem<Etp3Balance: HasCompact> {
	#[codec(compact)]
	pub value: Etp3Balance,
	#[codec(compact)]
...
pub struct UnappliedSlash<AccountId, Etp3Balance, DnaBalance> {
	/// The stash ID of the offending validator.
	validator: AccountId,
	/// The validator's own slash.
	own: slashing::RK<Etp3Balance, DnaBalance>,
	/// All other slashed stakers and amounts.
	others: Vec<(AccountId, slashing::RK<Etp3Balance, DnaBalance>)>,
	/// Reporters of the offence; bounty payout recipients.
	reporters: Vec<AccountId>,
	/// The amount of payout.
	payout: slashing::RK<Etp3Balance, DnaBalance>,
}
...
#[derive(PartialEq, Eq, Clone, Encode, Decode, RuntimeDebug)]
pub struct ElectionResult<AccountId, Etp3Balance: HasCompact, DnaBalance: HasCompact> {
	/// Flat list of validators who have been elected.
	elected_stashes: Vec<AccountId>,
	/// Flat list of new exposures, to be updated in the [`Exposure`] storage.
	exposures: Vec<(AccountId, Exposure<AccountId, Etp3Balance, DnaBalance>)>,
