...
//! impunity after some point, feaetp3 no further repercussions. For that reason, we
//! automatically "chill" validators and withdraw a nominator's nomination after a slashing event,
//! requietp3 them to re-enlist voluntarily (acknowledging the slash) and begin a new
//! slashing span.
//!
...
pub(crate) type RKT<T> = RK<Etp3Balance<T>, DnaBalance<T>>;
...
pub(crate) struct SpanRecord<Etp3Balance, DnaBalance> {
	slashed: RK<Etp3Balance, DnaBalance>,
	paid_out: RK<Etp3Balance, DnaBalance>,
}

impl<Etp3Balance, DnaBalance> SpanRecord<Etp3Balance, DnaBalance> {
	/// The value of stash balance slashed in this span.
	#[cfg(test)]
	pub(crate) fn amount_slashed(&self) -> &RK<Etp3Balance, DnaBalance> {
		&self.slashed
	}
}
...
pub(crate) struct SlashParams<'a, T: 'a + Trait> {
	...
	pub(crate) exposure: &'a Exposure<T::AccountId, Etp3Balance<T>, DnaBalance<T>>,
...
pub(crate) fn compute_slash<T: Trait>(
...
) -> Option<UnappliedSlash<T::AccountId, Etp3Balance<T>, DnaBalance<T>>> {
	let SlashParams {
		stash,
		slash,
		exposure,
	...
	let own_slash = RK {
		r: slash * exposure.own_etp3_balance,
		k: slash * exposure.own_dna_balance,
	};
	...
	for nominator in &exposure.others {
		...
		let era_slash = {
			let own_slash_prior = RK {
				r: prior_slash_p * nominator.etp3_balance,
				k: prior_slash_p * nominator.dna_balance,
			};
			let own_slash_by_validator = RK {
				r: slash * nominator.etp3_balance,
				k: slash * nominator.dna_balance,
			};
...
pub fn do_slash<T: Trait>(
	...
	slashed_etp3: &mut Etp3NegativeImbalance<T>,
	slashed_dna: &mut DnaNegativeImbalance<T>,
) {
	...
	let (slash_etp3, slash_dna) = ledger.slash(
		value.r,
		value.k,
	...
	if !slash_etp3.is_zero() {
		slashed = true;

		let (imbalance, missing) = T::Etp3Currency::slash(stash, slash_etp3);

		slashed_etp3.subsume(imbalance);
	...
	if !slash_dna.is_zero() {
		slashed = true;

		let (imbalance, missing) = T::DnaCurrency::slash(stash, slash_dna);

		slashed_dna.subsume(imbalance);
...
pub(crate) fn apply_slash<T: Trait>(
	unapplied_slash: UnappliedSlash<T::AccountId, Etp3Balance<T>, DnaBalance<T>>,
) {
	let mut slashed_etp3 = <Etp3NegativeImbalance<T>>::zero();
	let mut slashed_dna = <DnaNegativeImbalance<T>>::zero();
...
fn pay_reporters<T: Trait>(
	reward_payout: RKT<T>,
	slashed_etp3: Etp3NegativeImbalance<T>,
	slashed_dna: DnaNegativeImbalance<T>,
	reporters: &[T::AccountId],
) {
	if reporters.is_empty() || reward_payout.is_zero() {
		T::Etp3Slash::on_unbalanced(slashed_etp3);
		T::DnaSlash::on_unbalanced(slashed_dna);

		return;
	}

	// take rewards out of the slashed imbalance.
	let etp3_reward_payout = reward_payout.r.min(slashed_etp3.peek());
	let (mut etp3_reward_payout, mut etp3_slashed) = slashed_etp3.split(etp3_reward_payout);
	let dna_reward_payout = reward_payout.k.min(slashed_dna.peek());
	let (mut dna_reward_payout, mut dna_slashed) = slashed_dna.split(dna_reward_payout);

	let etp3_per_reporter = etp3_reward_payout.peek() / (reporters.len() as u32).into();
	let dna_per_reporter = dna_reward_payout.peek() / (reporters.len() as u32).into();

	for reporter in reporters {
		if !etp3_per_reporter.is_zero() {
			let (etp3_reporter_reward, etp3_rest) = etp3_reward_payout.split(etp3_per_reporter);
			etp3_reward_payout = etp3_rest;

			// this cancels out the reporter reward imbalance internally, leading
			// to no change in total issuance.
			T::Etp3Currency::resolve_creating(reporter, etp3_reporter_reward);
		}

		if !dna_per_reporter.is_zero() {
			let (dna_reporter_reward, dna_rest) = dna_reward_payout.split(dna_per_reporter);
			dna_reward_payout = dna_rest;

			// this cancels out the reporter reward imbalance internally, leading
			// to no change in total issuance.
			T::DnaCurrency::resolve_creating(reporter, dna_reporter_reward);
		}
	}

	// the rest goes to the on-slash imbalance handler (e.g. treasury)
	etp3_slashed.subsume(etp3_reward_payout); // remainder of reward division remains.
	T::Etp3Slash::on_unbalanced(etp3_slashed);

	// the rest goes to the on-slash imbalance handler (e.g. treasury)
	dna_slashed.subsume(dna_reward_payout); // remainder of reward division remains.
	T::DnaSlash::on_unbalanced(dna_slashed);
}
