...
/// `a` has 100 Etp3 and 100 Dna
/// promise for `m` month with 50 Etp3 and 50 Dna
///
...
macro_rules! gen_paired_account {
	($stash:ident($stash_id:expr), $controller:ident($controller_id:expr), $promise_month:ident($how_long:expr)) => {
		...
		let $stash = $stash_id;
		let _ = Etp3::deposit_creating(&$stash, 100 * COIN);
		let _ = Dna::deposit_creating(&$stash, 100 * COIN);
		#[allow(non_snake_case, unused)]
		let $controller = $controller_id;
		let _ = Etp3::deposit_creating(&$controller, COIN);
		#[allow(non_snake_case, unused)]
		let $promise_month = $how_long;
		assert_ok!(Staking::bond(
			Origin::signed($stash),
			$controller,
			StakingBalance::Etp3Balance(50 * COIN),
...
fn slash_ledger_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let _ = Etp3::deposit_creating(&account_id, bond);

			assert_ok!(Staking::bond(
				Origin::signed(account_id),
				account_id,
				StakingBalance::Etp3Balance(bond),
...

			{
				...
				assert_eq!(
					...
					StakingLedger {
						stash: account_id,
						active_etp3: total - total_slashed,
						active_deposit_etp3: deposit - deposit_slashed,
...
fn dna_should_reward_even_does_not_own_dna_before() {
	...
	ExtBuilder::default()
		...
		.execute_with(|| {
			let account_id = 777;
			let _ = Etp3::deposit_creating(&account_id, 10000);

			assert!(Dna::free_balance(&account_id).is_zero());
			assert_ok!(Staking::bond(
				Origin::signed(account_id),
				account_id,
				StakingBalance::Etp3Balance(10000),
				RewardDestination::Stash,
				36,
			));
...
#[test]
fn time_deposit_etp3_unbond_and_withdraw_automatically_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::unbond(
			...
			StakingBalance::Etp3Balance(unbond_value),
		));
		...
		assert_eq!(
			...
			StakingLedger {
				stash,
				active_etp3: 1000 - unbond_value,
				active_deposit_etp3: 0,
				active_dna: 0,
				deposit_items: vec![],
				etp3_staking_lock: StakingLock {
					staking_amount: 1000 - unbond_value,
		...
		assert_err!(
			Etp3::transfer(Origin::signed(stash), controller, 1),
		...
		assert_ok!(Etp3::transfer(Origin::signed(stash), controller, 1));
		...
		assert_ok!(Staking::unbond(
			Origin::signed(controller),
			StakingBalance::Etp3Balance(COIN)
		));

		assert_eq!(Etp3::locks(&stash).len(), 1);

		System::set_block_number(BondingDurationInBlockNumber::get() + unbond_start_2 + 1);
		assert_ok!(Staking::unbond(
			Origin::signed(controller),
			StakingBalance::Etp3Balance(10)
		));
...
fn normal_unbond_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let _ = Etp3::deposit_creating(&stash, 1000 * COIN);
		let start = System::block_number();

		{
			...
			ledger.active_etp3 += value;
			ledger.active_deposit_etp3 += value;
			...
			ledger.etp3_staking_lock.staking_amount += value;
...
fn punished_claim_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let bond_value = 10;
		let _ = Etp3::deposit_creating(&stash, 1000);
		let mut ledger = StakingLedger {
			stash,
			active_etp3: bond_value,
			active_deposit_etp3: bond_value,
			deposit_items: vec![TimeDepositItem {
				value: bond_value,
				start_time: INIT_TIMESTAMP,
				expire_time: INIT_TIMESTAMP + promise_month * MONTH_IN_MILLISECONDS,
		...
		assert_ok!(Staking::bond(
			Origin::signed(stash),
			controller,
			StakingBalance::Etp3Balance(bond_value),
			RewardDestination::Stash,
			promise_month as u8,
		));
		assert_eq!(Staking::ledger(controller).unwrap(), ledger);
		...
		ledger.active_deposit_etp3 -= bond_value;
...
fn deposit_zero_should_do_nothing() {
	ExtBuilder::default().build().execute_with(|| {
		let (stash, controller) = (1001, 1000);
		let _ = Etp3::deposit_creating(&stash, COIN);
		assert_ok!(Staking::bond(
			Origin::signed(stash),
			controller,
			StakingBalance::Etp3Balance(COIN),
...
#[test]
fn transform_to_deposited_etp3_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		let (stash, controller) = (1001, 1000);
		let _ = Etp3::deposit_creating(&stash, COIN);
		assert_ok!(Staking::bond(
		...
		ledger.active_deposit_etp3 += COIN;
		ledger.deposit_items.push(TimeDepositItem {
			value: COIN,
			start_time: INIT_TIMESTAMP,
...
#[test]
fn expired_etp3_should_capable_to_promise_again() {
	ExtBuilder::default().build().execute_with(|| {
		let (stash, controller) = (1001, 1000);
		let _ = Etp3::deposit_creating(&stash, 10);
		assert_ok!(Staking::bond(
			Origin::signed(stash),
			controller,
			StakingBalance::Etp3Balance(10),
		...
		ledger.active_deposit_etp3 = promise_extra_value;
...
fn inflation_should_be_correct() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let surplus_needed = initial_issuance - Etp3::total_issuance();
		let _ = Etp3::deposit_into_existing(&11, surplus_needed);

		assert_eq!(Etp3::total_issuance(), initial_issuance);
	});
...
fn slash_also_slash_unbondings() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let mut etp3_staking_lock = Staking::ledger(account_id)
				.unwrap()
				.etp3_staking_lock
				.clone();
			...
			assert_ok!(Staking::unbond(
				Origin::signed(account_id),
				StakingBalance::Etp3Balance(COIN / 2)
			));
			...
			etp3_staking_lock.staking_amount = 0;
			etp3_staking_lock.unbondings.clear();

			assert_eq!(
				Staking::ledger(account_id).unwrap().etp3_staking_lock,
				etp3_staking_lock
			);
		});
}
...
fn pool_should_be_increased_and_decreased_correctly() {
	ExtBuilder::default().build().execute_with(|| {
		...
		let mut etp3_pool = Staking::etp3_pool();
		...
		etp3_pool += 100 * COIN;
		dna_pool += 100 * COIN;
		assert_eq!(Staking::etp3_pool(), etp3_pool);
	...
	ExtBuilder::default()
		...
		.build_and_execute(|| {
			...
			let etp3_pool = Staking::etp3_pool();
			assert_ok!(Staking::payout_stakers(Origin::signed(10), 11, 1));
			assert_eq!(Staking::etp3_pool(), payout + etp3_pool);
		});
}
...
fn unbond_over_max_unbondings_chunks_should_fail() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_err!(
			Staking::unbond(Origin::signed(controller), StakingBalance::Etp3Balance(1)),
...
fn promise_extra_should_not_remove_unexpired_items() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::bond_extra(
			Origin::signed(stash),
			StakingBalance::Etp3Balance(5 * COIN),
			0,
		));
...
fn on_deposit_redeem_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		// Not bond yet
		{
			...
			assert_eq!(Etp3::free_balance(unbonded_account), 0);
			assert!(Etp3::locks(unbonded_account).is_empty());
			...
			assert_eq!(Etp3::free_balance(unbonded_account), deposit_amount);
			assert_eq!(
				Etp3::locks(unbonded_account),
				vec![BalanceLock {
					id: STAKING_ID,
					lock_for: LockFor::Staking(StakingLock {
			...
			assert_eq!(
				...
				StakingLedger {
					...
					active_etp3: deposit_amount,
					active_deposit_etp3: deposit_amount,
			...
			assert_eq!(Staking::etp3_pool(), etp3_pool + deposit_amount);
			assert!(System::account(unbonded_account).refcount != 0);
		}
		...
		// Already bonded
		{
			...
			assert_eq!(Etp3::free_balance(bonded_account), 101 * COIN);
			assert_eq!(Etp3::locks(bonded_account).len(), 1);
			...
			ledger.active_etp3 += deposit_amount;
			ledger.active_deposit_etp3 += deposit_amount;
			ledger.deposit_items.push(deposit_item);
			ledger.etp3_staking_lock.staking_amount += deposit_amount;

			assert_eq!(
				Etp3::free_balance(bonded_account),
				101 * COIN + deposit_amount
			);
			assert_eq!(
				Etp3::locks(bonded_account),
				vec![BalanceLock {
					id: STAKING_ID,
					lock_for: LockFor::Staking(StakingLock {
...
// bond 10_000 Etp3 for 12 months, gain 1 Dna
// bond extra 10_000 Etp3 for 36 months, gain 3 Dna
// bond extra 1 Dna
// nominate
// unlock the 12 months deposit item with punish
// lost 3 Dna and 10_000 Etp3's power for nominate
#[test]
fn two_different_bond_then_unbond_specific_one() {
	ExtBuilder::default().build().execute_with(|| {
		let (stash, controller) = (777, 888);
		let _ = Etp3::deposit_creating(&stash, 20_000);

		// Earn 1 Dna with bond 10_000 Etp3 12 months
		assert_ok!(Staking::bond(
			Origin::signed(stash),
			controller,
			StakingBalance::Etp3Balance(10_000),
			RewardDestination::Stash,
			12,
		));

		// Earn 3 Dna with bond 10_000 Etp3 36 months
		assert_ok!(Staking::bond_extra(
			Origin::signed(stash),
			StakingBalance::Etp3Balance(10_000),
			36,
...
fn staking_with_dna_with_unbondings() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Etp3::free_balance(stash), 10);
		assert_eq!(
			Etp3::locks(stash),
			vec![BalanceLock {
				id: STAKING_ID,
		...
		assert_eq!(Etp3::free_balance(stash), 9);
		assert_eq!(
			Etp3::locks(stash),
			vec![BalanceLock {
				id: STAKING_ID,
		...
		let _ = Etp3::deposit_creating(&stash, 20);
		assert_ok!(Staking::bond_extra(
			Origin::signed(stash),
			StakingBalance::Etp3Balance(19),
			0
		));
		assert_eq!(Etp3::free_balance(stash), 29);
		assert_eq!(
			Etp3::locks(stash),
			vec![BalanceLock {
				id: STAKING_ID,
...
fn unbound_values_in_twice() {
	...
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_ok!(Staking::unbond(
			Origin::signed(controller),
			StakingBalance::Etp3Balance(unbond_value_1)
		));
		...
		assert_err!(
			Etp3::transfer(Origin::signed(stash), controller, unbond_value_1),
			Etp3Error::LiquidityRestrictions,
		);
		...
		assert_eq!(Etp3::free_balance(stash), 9);
		assert_err!(
			Etp3::transfer(Origin::signed(stash), controller, unbond_value_1 + 1),
			Etp3Error::LiquidityRestrictions,
		);
		System::set_block_number(BondingDurationInBlockNumber::get() + unbond_start_1);
		assert_ok!(Etp3::transfer(
		...
		assert_eq!(Etp3::free_balance(stash), 7);
		assert_eq!(
			Etp3::locks(stash),
			vec![BalanceLock {
				id: STAKING_ID,
		...
		assert_eq!(Etp3::free_balance(stash), 1);
		assert_eq!(
			Etp3::locks(stash),
			vec![BalanceLock {
				id: STAKING_ID,
		...
		let _ = Etp3::deposit_creating(&stash, 1);
		//		println!("Staking Ledger: {:#?}", Staking::ledger(controller).unwrap());
		assert_eq!(Etp3::free_balance(stash), 2);
		assert_ok!(Staking::bond_extra(
			Origin::signed(stash),
			StakingBalance::Etp3Balance(1),
			0
...
fn rebond_event_should_work() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			assert_eq!(
				staking_events(),
				vec![RawEvent::BondEtp3(200, 36000, 36000)]
			);
		});
}
