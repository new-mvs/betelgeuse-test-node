...
pub fn compute_total_payout<T: Trait>(
	era_duration: TsInMs,
	living_time: TsInMs,
	total_left: Etp3Balance<T>,
	payout_fraction: Perbill,
) -> (Etp3Balance<T>, Etp3Balance<T>) {
	info!(
		target: "betelgeuse-staking",
	...
	(
		<Etp3Balance<T>>::saturated_from::<Balance>(payout),
		<Etp3Balance<T>>::saturated_from::<Balance>(inflation),
	)
}
...
// https://github.com/evolutionlandorg/bank/blob/master/contracts/Getp3ottsBank.sol#L280
pub fn compute_dna_reward<T: Trait>(value: Etp3Balance<T>, months: u8) -> DnaBalance<T> {
