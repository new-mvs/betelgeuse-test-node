...
//!
//! `TermDuration` might change duetp3 a round. This can shorten or extend the length of the round.
...
//! (voting for non-candidates) are ignored duetp3 election. Yet, a voter _might_ vote for a future
...
decl_storage! {
	trait Store for Module<T: Trait> as BetelgeusePhragmenElection {
		// ---- State
...
mod tests {
	...
	frame_support::construct_runtime!(
			...
			UncheckedExtrinsic = UncheckedExtrinsic
		{
			...
			Balances: betelgeuse_balances::<Instance0>::{Module, Call, Config<T>, Event<T>},
			Elections: elections_phragmen::{Module, Call, Config<T>, Event<T>},
		}
	);
	...
	impl ExtBuilder {
		...
		pub fn build_and_execute(self, test: impl FnOnce() -> ()) {
			...
			let mut ext: sp_io::TestExternalities = GenesisConfig {
				betelgeuse_balances_Instance0: Some(betelgeuse_balances::GenesisConfig::<
					Test,
					Etp3Instance,
				> {
					balances: vec![
	...
	fn all_voters() -> Vec<u64> {
		<Voting<Test>>::iter().map(|(v, _)| v).collect()
	}
