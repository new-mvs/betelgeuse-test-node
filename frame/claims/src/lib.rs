...
pub trait Trait: frame_system::Trait {
	...
	/// The *RING* currency.
	type Etp3Currency: LockableCurrency<Self::AccountId>;
...
decl_event!(
		...
		Etp3Balance = Etp3Balance<T>,
	{
		...
		Claimed(AccountId, AddressT, Etp3Balance),
	}
);
...
decl_storage! {
	trait Store for Module<T: Trait> as BetelgeuseClaims {
		ClaimsFromEth
			get(fn claims_from_eth)
			: map hasher(identity) AddressT => Option<Etp3Balance<T>>;
		ClaimsFromTron
	...
	add_extra_genesis {
		...
		build(|config| {
			...
			let mut total = <Etp3Balance<T>>::zero();

			if dot.is_empty() && eth.is_empty() && tron.is_empty() {
				error!("[betelgeuse-claims] Genesis Claims List is Set to EMPTY");
			} else {
				// Eth Address
				for Account { address, backed_etp3 } in dot {
					// DOT:RING = 1:50
					let backed_etp3 = (*backed_etp3).saturated_into();
					<ClaimsFromEth<T>>::insert(address.0, backed_etp3);
					total += backed_etp3;
				}
				for Account { address, backed_etp3 } in eth {
					let backed_etp3 = (*backed_etp3).saturated_into();
					<ClaimsFromEth<T>>::insert(address.0, backed_etp3);
					total += backed_etp3;
				}
				...
				// Tron Address
				for Account { address, backed_etp3 } in tron {
					let backed_etp3 = (*backed_etp3).saturated_into();
					<ClaimsFromTron<T>>::insert(address.0, backed_etp3);
					total += backed_etp3;
				}
			}

			let minimum_balance = T::Etp3Currency::minimum_balance();
...
decl_module! {
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
			...
			match signature {
				OtherSignature::Eth(signature) => {
					...
					let balance_due = <ClaimsFromEth<T>>::get(&signer)
						...
						Self::pot::<T::Etp3Currency>() >= balance_due,
		...
		fn mint_claim(origin, who: OtherAddress, value: Etp3Balance<T>) {
			ensure_root(origin)?;

			match who {
				OtherAddress::Eth(who) => {
					T::Etp3Currency::deposit_creating(&Self::account_id(), value);
...
mod tests {
	...
	fn new_test_ext() -> sp_io::TestExternalities {
		...
		betelgeuse_balances::GenesisConfig::<Test, Etp3Instance>::default()
	...
	fn basic_setup_works() {
		new_test_ext().execute_with(|| {
			assert_eq!(Etp3::usable_balance(&Claims::account_id()), 600);
	...
	fn claiming_works() {
		new_test_ext().execute_with(|| {
			...
			assert_eq!(Etp3::usable_balance(&Claims::account_id()), 500);
			...
			assert_eq!(Etp3::free_balance(&2), 200);
			assert_eq!(Etp3::usable_balance(&Claims::account_id()), 300);
			...
			assert_eq!(Etp3::free_balance(&3), 300);
			assert_eq!(Etp3::usable_balance(&Claims::account_id()), 0);
		});
	}
	...
	fn basic_claim_moving_works() {
		new_test_ext().execute_with(|| {
			...
			assert_eq!(Etp3::free_balance(&42), 100);
			assert_eq!(
				Etp3::usable_balance(&Claims::account_id()),
				total_claims() - 100
			);
		});
	}
	...
	fn add_claim_works() {
		new_test_ext().execute_with(|| {
			...
			assert_eq!(Etp3::usable_balance(&Claims::account_id()), 800);
			assert_ok!(Claims::claim(
				Origin::none(),
				69,
				OtherSignature::Eth(eth_sig::<Test>(
			...
			assert_eq!(Etp3::free_balance(&69), 200);
			assert_eq!(Etp3::usable_balance(&Claims::account_id()), 600);
		});
	}
