...
macro_rules! impl_account_data {
	(
		...
		use betelgeuse_support::balance::BalanceInfo;
		...
		impl BalanceInfo<$btype, $etp3_instance> for AccountData<$btype> {
			fn free(&self) -> $btype {
				self.free
			}
			...
			fn usable(
				&self,
				reasons: betelgeuse_support::balance::lock::LockReasons,
				frozen_balance: betelgeuse_support::balance::FrozenBalance<$btype>,
			) -> $btype {
