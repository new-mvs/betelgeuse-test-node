...
impl<T: Trait> Module<T> {
	...
	pub fn matuetp3_referenda_at(
		...
		Self::matuetp3_referenda_at_inner(n, next..last)
	}

	fn matuetp3_referenda_at_inner(
		n: T::BlockNumber,
	...
	///
	/// # <weight>
	/// If a referendum is launched or matuetp3, this will take full block weight. Otherwise:
	...
	fn begin_block(now: T::BlockNumber) -> Result<Weight, DispatchError> {
		...
		// tally up votes for any expietp3 referenda.
		for (index, info) in Self::matuetp3_referenda_at_inner(now, next..last).into_iter() {
