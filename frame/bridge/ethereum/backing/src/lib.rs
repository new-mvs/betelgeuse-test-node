...
// TODO: https://github.com/betelgeuse-network/betelgeuse-common/issues/372
#![allow(unused)]
...
mod types {
	...
	pub type Etp3Balance<T> = <<T as Trait>::Etp3Currency as Currency<AccountId<T>>>::Balance;
...
use betelgeuse_relay_primitives::relay_authorities::*;
...
pub trait Trait: frame_system::Trait {
	...
	type EthereumRelay: EthereumReceipt<Self::AccountId, Etp3Balance<Self>>;
	type OnDepositRedeem: OnDepositRedeem<Self::AccountId, Etp3Balance<Self>>;
	...
	type Etp3LockLimit: Get<Etp3Balance<Self>>;
	type DnaLockLimit: Get<DnaBalance<Self>>;
	type AdvancedFee: Get<Etp3Balance<Self>>;
	type SyncReward: Get<Etp3Balance<Self>>;
...
// TODO: https://github.com/betelgeuse-network/betelgeuse-common/issues/209
...
decl_event! {
		...
		DnaBalance = DnaBalance<T>,
	{
		...
		RedeemEtp3(AccountId, Balance, EthereumTransactionIndex),
		...
		RedeemDeposit(AccountId, DepositId, Etp3Balance, EthereumTransactionIndex),
		/// Someone lock some *RING*. [account, ethereum account, asset address, amount]
		LockEtp3(AccountId, EthereumAddress, EthereumAddress, Etp3Balance),
...
decl_storage! {
	trait Store for Module<T: Trait> as BetelgeuseEthereumBacking {
		pub VerifiedProof
		...
		pub Etp3TokenAddress get(fn etp3_token_address) config(): EthereumAddress;
...
decl_module! {
		...
		origin: T::Origin
	{
		...
		const AdvancedFee: Etp3Balance<T> = T::AdvancedFee::get();

		const SyncReward: Etp3Balance<T> = T::SyncReward::get();
		...
		pub fn lock(
			origin,
			#[compact] etp3_value: Etp3Balance<T>,
			#[compact] dna_value: DnaBalance<T>,
			...
			// 50 Etp3 for fee
			// https://github.com/betelgeuse-network/betelgeuse-common/pull/377#issuecomment-730369387
			T::Etp3Currency::transfer(&user, &fee_account, T::AdvancedFee::get(), KeepAlive)?;

			let mut locked = false;

			if !etp3_value.is_zero() {
				let etp3_to_lock = etp3_value.min(T::Etp3Currency::usable_balance(&user));

				ensure!(etp3_to_lock < T::Etp3LockLimit::get(), <Error<T>>::Etp3LockLim);

				T::Etp3Currency::transfer(&user, &Self::account_id(), etp3_to_lock, KeepAlive)?;

				let raw_event = RawEvent::LockEtp3(
					user.clone(),
					ethereum_account.clone(),
					Etp3TokenAddress::get(),
					etp3_to_lock
				);
			...
			if !dna_value.is_zero() {
				let dna_to_lock = dna_value.min(T::DnaCurrency::usable_balance(&user));

				ensure!(dna_to_lock < T::DnaLockLimit::get(), <Error<T>>::DnaLockLim);

				T::DnaCurrency::transfer(&user, &Self::account_id(), dna_to_lock, KeepAlive)?;
		...
		fn sync_authorities_change(origin, proof: EthereumReceiptProofThing<T>) {
			...
			let sync_reward = T::SyncReward::get().min(
				T::Etp3Currency::usable_balance(&fee_account)
					.saturating_sub(T::Etp3Currency::minimum_balance())
			);

			if !sync_reward.is_zero() {
				T::Etp3Currency::transfer(
					&fee_account,
					&beneficiary,
					sync_reward,
					KeepAlive
		...
		/// Set a new etp3 redeem address.
		///
		/// The dispatch origin of this call must be _Root_.
		///
		/// - `new`: The new etp3 redeem address.
		///
		/// # <weight>
		/// - `O(1)`.
		/// # </weight>
...
impl<T: Trait> Module<T> {
	...
	fn parse_token_redeem_proof(
	...
	) -> Result<(T::AccountId, (bool, Balance), Etp3Balance<T>), DispatchError> {
		...
		let is_etp3 = {
			let token_address = result.params[0]
				...
				token_address == Etp3TokenAddress::get()
		...
		debug::trace!(target: "ethereum-backing", "[ethereum-backing] Betelgeuse Account: {:?}", betelgeuse_account);

		Ok((betelgeuse_account, (is_etp3, redeemed_amount), fee))
	}
	...
	> {
		...
		let redeemed_etp3 = {
			// The decimal in Ethereum is 10**18, and the decimal in Betelgeuse is 10**9,
			// div 10**18 and mul 10**9
			let redeemed_etp3 = result.params[5]
				.value
				.clone()
				.to_uint()
				.map(|x| x / U256::from(1_000_000_000u64))
			...
			<Etp3Balance<T>>::saturated_from(redeemed_etp3.saturated_into())
		};
		let betelgeuse_account = {
	...
	// https://github.com/betelgeuse-network/betelgeuse-bridge-on-ethereum/blob/51839e614c0575e431eabfd5c70b84f6aa37826a/contracts/Relay.sol#L22
	...
	fn redeem_token(
		...
		// TODO: remove fee?
		let (betelgeuse_account, (is_etp3, redeem_amount), fee) =
			Self::parse_token_redeem_proof(&proof)?;

		if is_etp3 {
			Self::redeem_token_cast::<T::Etp3Currency>(
				redeemer,
				betelgeuse_account,
				tx_index,
				true,
	...
	fn redeem_token_cast<C: LockableCurrency<T::AccountId>>(
		redeemer: &T::AccountId,
		betelgeuse_account: T::AccountId,
		...
		// ensure!(
		// 	T::Etp3Currency::usable_balance(redeemer) >= fee,
		// 	<Error<T>>::FeeIns
		// );
		...
		// T::Etp3Currency::transfer(redeemer, &T::EthereumRelay::account_id(), fee, KeepAlive)?;

		VerifiedProof::insert(tx_index, true);

		if is_etp3 {
			Self::deposit_event(RawEvent::RedeemEtp3(betelgeuse_account, raw_amount, tx_index));
		} else {
			Self::deposit_event(RawEvent::RedeemDna(betelgeuse_account, raw_amount, tx_index));
		}
	...
	fn redeem_deposit(
		...
		// TODO: remove fee?
		let (deposit_id, betelgeuse_account, redeemed_etp3, start_at, months, fee) =
			Self::parse_deposit_redeem_proof(&proof)?;

		ensure!(
			Self::pot::<T::Etp3Currency>() >= redeemed_etp3,
			<Error<T>>::Etp3LockedNSBA
		);
