...
//! # Betelgeuse Ethereum Relay Module
...
mod types {
	...
	pub type Etp3Balance<T> = <CurrencyT<T> as Currency<AccountId<T>>>::Balance;
...
#[cfg(feature = "std")]
betelgeuse_support::impl_genesis! {
	struct DagsMerkleRootsLoader {
		dags_merkle_roots: Vec<H128>
	}
}
decl_storage! {
	trait Store for Module<T: Trait> as BetelgeuseEthereumRelay {
		...
		/// The highest ethereum block number that record in betelgeuse
...
decl_module! {
		...
		origin: T::Origin
	{
		...
		/// receipt, we need to provide this as part of proof, because in Betelgeuse Relay, we only have
		...
		/// # </weight>
		#[weight = 10_000_000]
		pub fn set_receipt_verify_fee(origin, #[compact] new: Etp3Balance<T>) {
...
impl<T: Trait> EthereumReceiptT<AccountId<T>, Etp3Balance<T>> for Module<T> {
	...
	fn receipt_verify_fee() -> Etp3Balance<T> {
