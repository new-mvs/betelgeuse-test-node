...
//!
//! In this module,
//! the offchain worker will keep fetch the next block info and relay to Betelgeuse Network.
...
//! Now the shadow service is provided by our another project, betelgeuse.js.
//! https://github.com/betelgeuse-network/betelgeuse.js
//!
//!
...
//! - After all, the corrected Ethereum header with the proofs will be submit and validate on chain of Betelgeuse Network by
//!   `submit_header`
//!
...
//! https://github.com/betelgeuse-network/betelgeuse-common/issues/86
//!
//! More details about offchain workers in following PRs
//! https://github.com/betelgeuse-network/betelgeuse/pull/335
//! https://github.com/betelgeuse-network/betelgeuse-common/pull/43
//! https://github.com/betelgeuse-network/betelgeuse-common/pull/63
...
use betelgeuse_support::literal_procesor::extract_from_json_str;
use ethereum_primitives::{ethashproof::EthashProof, header::EthereumHeader};

type EthereumRelay<T> = betelgeuse_ethereum_linear_relay::Module<T>;
type EthereumRelayCall<T> = betelgeuse_ethereum_linear_relay::Call<T>;

pub const ETH_OFFCHAIN: KeyTypeId = KeyTypeId(*b"etho");

/// A dummy endpoint, point this to shadow service
const ETH_RESOURCE: &'static [u8] = b"http://shadow.betelgeuse.network/";

pub trait Trait:
	CreateSignedTransaction<EthereumRelayCall<Self>> + betelgeuse_ethereum_linear_relay::Trait
{
...
impl<T: Trait> Module<T> {
	...
	/// Submit and record the valid header on Betelgeuse network
	fn submit_header(
		...
		// https://github.com/betelgeuse-network/betelgeuse-common/issues/137
		let results = {
			#[cfg(test)]
			{
				let _ = signer;
