	...
	for estimate_stake in 1..=3 {
		ExtBuilder::default()
			...
			.execute_with(|| {
				...
				assert!(Etp3::locks(1).is_empty());

				assert_eq!(RelayerGame::stakes_of(relayer_b), 0);
				assert!(Etp3::locks(2).is_empty());
			});
	}
}
	...
	for estimate_stake in vec![1, 5, 10, 20, 50, 100] {
		ExtBuilder::default()
			...
			.execute_with(|| {
				...
				assert_eq!(Etp3::usable_balance(&relayer_a), 1000);
				assert_eq!(Etp3::usable_balance(&relayer_b), 2000);
				...
				assert_eq!(Etp3::usable_balance(&relayer_a), 1000 + stakes);
				assert!(Etp3::locks(relayer_a).is_empty());

				assert_eq!(Etp3::usable_balance(&relayer_b), 2000 - stakes);
				assert!(Etp3::locks(relayer_b).is_empty());
			});
	}
}
...
fn settle_without_challenge_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		{
			assert_ok!(RelayerGame::affirm(&1, relay_header_parcel.clone(), None));
			assert!(Etp3::usable_balance(&1) < 100);
			assert!(!Etp3::locks(1).is_empty());
			...
			assert_eq!(Etp3::usable_balance(&1), 100);
			assert!(Etp3::locks(1).is_empty());
		}
	})
}
...
fn settle_abandon_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Etp3::usable_balance(&relayer_a), 100);
		assert_eq!(Etp3::usable_balance(&relayer_b), 200);
		...
		assert_eq!(Etp3::usable_balance(&relayer_a), 100 - 4);
		assert!(Etp3::locks(relayer_a).is_empty());

		assert_eq!(Etp3::usable_balance(&relayer_b), 200 - 4);
		assert!(Etp3::locks(relayer_b).is_empty());
	});
}
...
fn no_honesty_should_work() {
	ExtBuilder::default().build().execute_with(|| {
		...
		assert_eq!(Etp3::usable_balance(&relayer_a), 100 - 5);
		assert!(Etp3::locks(relayer_a).is_empty());

		assert_eq!(Etp3::usable_balance(&relayer_b), 200 - 5);
		assert!(Etp3::locks(relayer_b).is_empty());
	});
}
