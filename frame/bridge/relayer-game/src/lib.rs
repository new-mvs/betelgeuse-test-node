...
decl_storage! {
	trait Store for Module<T: Trait<I>, I: Instance = DefaultInstance> as BetelgeuseRelayerGame {
...
impl<T: Trait<I>, I: Instance> Module<T, I> {
	...
	) -> Result<Etp3Balance<T, I>, DispatchError> {
		let stake = T::RelayerGameAdjustor::estimate_stake(round, affirmations_count);

		ensure!(
			T::Etp3Currency::usable_balance(relayer) >= stake,
			<Error<T, I>>::StakeIns
		);
	...
	where
		F: FnOnce(Etp3Balance<T, I>) -> Etp3Balance<T, I>,
	{
		let stakes = calc_stakes(Self::stakes_of(relayer));

		if stakes.is_zero() {
			T::Etp3Currency::remove_lock(T::LockId::get(), relayer);
	...
	pub fn slash_on(relayer: &AccountId<T>, stake: Etp3Balance<T, I>) {
		Self::update_stakes_with(relayer, |old_stakes| old_stakes.saturating_sub(stake));

		T::Etp3Slash::on_unbalanced(T::Etp3Currency::slash(relayer, stake).0);
	}

	pub fn payout_honesties_and_slash_evils(
		honesties: BTreeMap<AccountId<T>, (Etp3Balance<T, I>, Etp3Balance<T, I>)>,
		evils: BTreeMap<AccountId<T>, Etp3Balance<T, I>>,
	) {
		for (relayer, (unstakes, rewards)) in honesties {
			...
			// Reward honesty
			T::Etp3Currency::deposit_creating(&relayer, rewards);
		}
		...
		for (relayer, slashs) in evils {
			...
			// Punish evil
			T::Etp3Currency::slash(&relayer, slashs);
		}
	}
	...
	pub fn settle_with_challenge(
		...
		let mut honesties = <BTreeMap<AccountId<T>, (Etp3Balance<T, I>, Etp3Balance<T, I>)>>::new();
		// BTreeMap<(relayer, slash)>
		let mut evils = <BTreeMap<AccountId<T>, Etp3Balance<T, I>>>::new();
		...
		if let Some(last_round_relay_affirmations) = relay_affirmations.last() {
			...
			match last_round_winning_relay_chain_indexes.len() {
				...
				1 => {
					...
					// BTreeMap<(relayer, unstake, reward)>
					let mut honesties =
						<BTreeMap<AccountId<T>, (Etp3Balance<T, I>, Etp3Balance<T, I>)>>::new();
					// BTreeMap<(relayer, slash)>
					let mut evils = <BTreeMap<AccountId<T>, Etp3Balance<T, I>>>::new();
