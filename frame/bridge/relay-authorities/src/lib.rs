...
mod types {
	...
	pub type Etp3Currency<T, I> = <T as Trait<I>>::Etp3Currency;
	...
	pub type RelayAuthorityT<T, I> =
		RelayAuthority<AccountId<T>, RelayAuthoritySigner<T, I>, Etp3Balance<T, I>, BlockNumber<T>>;
...
pub trait Trait<I: Instance = DefaultInstance>: frame_system::Trait {
	...
	type BetelgeuseMMR: MMR<Self::BlockNumber, Self::Hash>;
...
decl_error! {
	pub enum Error for Module<T: Trait<I>, I: Instance> {
		...
		ScheduledSignNE,
		/// Betelgeuse MMR Root - NOT READY YET
		BetelgeuseMMRRootNRY,
		/// Signature - INVALID
		SignatureInv,
		/// Term - MISMATCHED
		TermMis,
...
decl_storage! {
	trait Store for Module<T: Trait<I>, I: Instance = DefaultInstance> as BetelgeuseRelayAuthorities {
	...
	add_extra_genesis {
		config(authorities): Vec<(AccountId<T>, RelayAuthoritySigner<T, I>, Etp3Balance<T, I>)>;
		build(|config| {
		...
		origin: T::Origin
	{
		...
		pub fn request_authority(
			...
			if let Some(scheduled_authorities_change) = <NextAuthorities<T, I>>::get() {
				...
				<Etp3Currency<T, I>>::usable_balance(&account_id) > stake,
				<Error<T, I>>::StakeIns
			);
			...
			<Candidates<T, I>>::try_mutate(|candidates| {
				...
				if candidates.len() == T::MaxCandidates::get() {
					...
					<Etp3Currency<T, I>>::remove_lock(T::LockId::get(), &weep_out.account_id);
				}

				<Etp3Currency<T, I>>::set_lock(
					T::LockId::get(),
					&account_id,
					LockFor::Common { amount: stake },
					WithdrawReasons::all()
		...
		pub fn cancel_request(origin) {
			...
			let _ = Self::remove_candidate_by_id_with(
				&account_id,
				|| <Etp3Currency<T, I>>::remove_lock(T::LockId::get(), &account_id)
			);
		}
		...
		pub fn kill_candidates(origin) {
			...
			for RelayAuthority { account_id, .. } in <Candidates<T, I>>::take() {
				<Etp3Currency<T, I>>::remove_lock(lock_id, &account_id);
			}
		}
		...
		///
		/// This call is disallowed duetp3 the authorities change
		#[weight = 10_000_000]
		...
		pub fn submit_signed_mmr_root(
			...
			// Not allow to submit duetp3 the authority set change
			ensure!(!Self::on_authorities_change(), <Error<T, I>>::OnAuthoritiesChangeDis);
			...
			let mmr_root =
				T::BetelgeuseMMR::get_root(block_number).ok_or(<Error<T, I>>::BetelgeuseMMRRootNRY)?;
		...
		/// Require authority origin
		///
		/// This call is only allowed duetp3 the authorities change
		///
		/// No-op if already submit
		///
		/// Verify
			...
			|signatures: Vec<(AccountId<T>, RelayAuthoritySignature<T, I>)>| {
				...
				} in <Authorities<T, I>>::get()
				{
					if let None = signatures
						...
						.position(|(authority, _)| authority == &account_id)
					{
						<Etp3Currency<T, I>>::slash(&account_id, stake);
