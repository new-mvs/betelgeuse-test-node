...
use betelgeuse_evm::{AccountBasicMapping, AddressMapping, GasToWeight, Runner};
use betelgeuse_evm_primitives::CallOrCreateInfo;
...
pub type BalanceOf<T> = <T as betelgeuse_balances::Trait>::Balance;
type Etp3Instance = betelgeuse_balances::Instance0;

/// Trait for Ethereum pallet.
pub trait Trait:
	frame_system::Trait<Hash = H256>
	+ betelgeuse_balances::Trait<Etp3Instance>
	+ pallet_timestamp::Trait
	+ betelgeuse_evm::Trait
{
	...
	type Etp3Currency: Currency<Self::AccountId>;
}
...
decl_module! {
	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		#[weight = <T as betelgeuse_evm::Trait>::GasToWeight::gas_to_weight(transaction.gas_limit.low_u32())]
...
impl<T: Trait> frame_support::unsigned::ValidateUnsigned for Module<T> {
	...
	fn validate_unsigned(_source: TransactionSource, call: &Self::Call) -> TransactionValidity {
		if let Call::transact(transaction) = call {
			...
			let account_data =
				<T as betelgeuse_evm::Trait>::AccountBasicMapping::account_basic(&origin);
