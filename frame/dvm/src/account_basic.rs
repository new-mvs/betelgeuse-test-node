use betelgeuse_evm::{Account as EVMAccount, AccountBasicMapping, AddressMapping};
...
impl<T: crate::Trait + betelgeuse_balances::Trait<betelgeuse_balances::Instance0>> AccountBasicMapping
	for DVMAccountBasicMapping<T>
{
	/// Get the account basic in EVM format.
	fn account_basic(address: &H160) -> EVMAccount {
		let account_id = <T as betelgeuse_evm::Trait>::AddressMapping::into_account_id(*address);
	...
	fn mutate_account_basic(address: &H160, new: EVMAccount) {
		...
		if current.balance > new.balance {
			...
			if dvm_balance < diff_remaining_balance {
				...
				let value = <T as betelgeuse_balances::Trait<betelgeuse_balances::Instance0>>::Balance::unique_saturated_from(
