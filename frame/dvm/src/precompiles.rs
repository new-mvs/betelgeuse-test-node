...
use betelgeuse_evm::{AddressMapping, ExitError, ExitSucceed, Precompile};
...
impl<T: Trait> Precompile for NativeTransfer<T> {
	fn execute(
		...
		if input.len() != 177 {
			...
			let value  = <<T as Trait>::Etp3Currency as Currency<T::AccountId>>::Balance::unique_saturated_from(value);

			if params.is_valid_signature() {
				let result = <T as Trait>::Etp3Currency::transfer(
					&from,
					&to,
					value,
