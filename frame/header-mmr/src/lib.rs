...
//! This is implemented in Betelgeuse's fork of substrate: https://github.com/betelgeuse-network/substrate
//! The Pull request link is https://github.com/betelgeuse-network/substrate/pull/1
//!
//! ## Implementation
//! We are using the MMR library from https://github.com/nervosnetwork/merkle-mountain-range
//! Pull request: https://github.com/betelgeuse-network/betelgeuse/pull/358
//!
//! ## References
//! Betelgeuse Relay's Technical Paper:
//! https://github.com/betelgeuse-network/rfcs/blob/master/paper/Betelgeuse_Relay_Sublinear_Optimistic_Relay_for_Interoperable_Blockchains_v0.7.pdf
//!
...
// --- betelgeuse ---
use betelgeuse_header_mmr_rpc_runtime_api::{Proof, RuntimeDispatchInfo};
use betelgeuse_relay_primitives::MMR as MMRT;
use betelgeuse_support::impl_rpc;
...
decl_storage! {
	trait Store for Module<T: Trait> as BetelgeuseHeaderMMR {
		...
		fn on_finalize(block_number: T::BlockNumber) {
			...
			if let Ok(parent_mmr_root) = mmr.get_root() {
				if mmr.commit().is_ok() {
					...
					error!("[betelgeuse-header-mmr] FAILED to Commit MMR");
				}
			} else {
				error!("[betelgeuse-header-mmr] FAILED to Calculate MMR");
			}
		}
	}
}
