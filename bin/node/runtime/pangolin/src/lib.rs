//! The Betelgeuse Node Template runtime. This can be compiled with `#[no_std]`, ready for Wasm.
...
pub mod impls {
	...
	pub mod relay {
		// --- betelgeuse ---
		use crate::*;
		use betelgeuse_relay_primitives::relayer_game::*;
	...
	betelgeuse_support::impl_account_data! {
	...
	impl OnUnbalanced<NegativeImbalance> for Author {
		fn on_nonzero_unbalanced(amount: NegativeImbalance) {
			Etp3::resolve_creating(&Authorship::author(), amount);
		}
	}
...
pub use betelgeuse_staking::StakerStatus;
pub use wasm::*;
...
// --- betelgeuse ---
use constants::*;
use betelgeuse_balances_rpc_runtime_api::RuntimeDispatchInfo as BalancesRuntimeDispatchInfo;
use betelgeuse_evm::Runner;
use betelgeuse_header_mmr_rpc_runtime_api::RuntimeDispatchInfo as HeaderMMRRuntimeDispatchInfo;
use betelgeuse_relay_primitives::relay_authorities::OpCode;
use betelgeuse_staking::EraIndex;
use betelgeuse_staking_rpc_runtime_api::RuntimeDispatchInfo as StakingRuntimeDispatchInfo;
use drml_primitives::*;
use dvm_ethereum::{
...
type SignedExtra = (
	...
	betelgeuse_ethereum_relay::CheckEthereumRelayHeaderParcel<Runtime>,
);
...
type NegativeImbalance = <betelgeuse_balances::Module<Runtime, Etp3Instance> as Currency<
...
type Etp3Instance = betelgeuse_balances::Instance0;
parameter_types! {
	pub const ExistentialDeposit: Balance = 1 * COIN;
	pub const MaxLocks: u32 = 50;
}
impl betelgeuse_balances::Trait<Etp3Instance> for Runtime {
	type Balance = Balance;
	type DustRemoval = ();
	type Event = Event;
	...
	type WeightInfo = weights::betelgeuse_balances::WeightInfo<Runtime>;
}
type DnaInstance = betelgeuse_balances::Instance1;
impl betelgeuse_balances::Trait<DnaInstance> for Runtime {
	type Balance = Balance;
	type DustRemoval = ();
	type Event = Event;
	type ExistentialDeposit = ExistentialDeposit;
...
impl betelgeuse_staking::Trait for Runtime {
	type Event = Event;
	...
	type Etp3RewardRemainder = Treasury;
	// send the slashed funds to the treasury.
	type Etp3Slash = Treasury;
	...
	type WeightInfo = weights::betelgeuse_staking::WeightInfo<Runtime>;
}
...
impl pallet_session::historical::Trait for Runtime {
	type FullIdentification = betelgeuse_staking::Exposure<AccountId, Balance, Balance>;
	type FullIdentificationOf = betelgeuse_staking::ExposureOf<Runtime>;
}
...
impl pallet_session::Trait for Runtime {
	...
	type ValidatorIdOf = betelgeuse_staking::StashOf<Self>;
...
impl betelgeuse_democracy::Trait for Runtime {
	type Proposal = Call;
	type Event = Event;
	type Currency = Etp3;
	...
	type WeightInfo = weights::betelgeuse_democracy::WeightInfo<Runtime>;
}
...
impl betelgeuse_elections_phragmen::Trait for Runtime {
	type Event = Event;
	...
	type WeightInfo = weights::betelgeuse_elections_phragmen::WeightInfo<Runtime>;
}
...
parameter_types! {
	...
	pub const Etp3ProposalBondMinimum: Balance = 20 * COIN;
...
impl betelgeuse_treasury::Trait for Runtime {
	...
	type Etp3BurnDestination = ();
	type DnaBurnDestination = ();
	type WeightInfo = weights::betelgeuse_treasury::WeightInfo<Runtime>;
}
...
impl betelgeuse_claims::Trait for Runtime {
	type Event = Event;
...
impl betelgeuse_vesting::Trait for Runtime {
	type Event = Event;
	type Currency = Etp3;
...
impl betelgeuse_crab_issuing::Trait for Runtime {
	type Event = Event;
...
impl betelgeuse_crab_backing::Trait for Runtime {
...
impl betelgeuse_ethereum_relay::Trait for Runtime {
...
type EthereumRelayerGameInstance = betelgeuse_relayer_game::Instance0;
parameter_types! {
	pub const EthereumRelayerGameLockId: LockIdentifier = *b"ethrgame";
}
impl betelgeuse_relayer_game::Trait<EthereumRelayerGameInstance> for Runtime {
	type Etp3Currency = Etp3;
...
impl betelgeuse_tron_backing::Trait for Runtime {
...
impl betelgeuse_header_mmr::Trait for Runtime {}
...
impl betelgeuse_evm::Trait for Runtime {
	...
	type Event = Event;
	type Precompiles = (
		betelgeuse_evm::precompiles::ECRecover,
		betelgeuse_evm::precompiles::Sha256,
		betelgeuse_evm::precompiles::Ripemd160,
		betelgeuse_evm::precompiles::Identity,
		NativeTransfer<Self>,
	);
	type ChainId = ChainId;
	type AccountBasicMapping = DVMAccountBasicMapping<Self>;
	type Runner = betelgeuse_evm::runner::stack::Runner<Self>;
}

type EthereumRelayAuthoritiesInstance = betelgeuse_relay_authorities::Instance0;
parameter_types! {
...
impl betelgeuse_relay_authorities::Trait<EthereumRelayAuthoritiesInstance> for Runtime {
	type Event = Event;
	type Etp3Currency = Etp3;
...
construct_runtime!(
		...
		UncheckedExtrinsic = UncheckedExtrinsic
	{
		...
		Balances: betelgeuse_balances::<Instance0>::{Module, Call, Storage, Config<T>, Event<T>},
		Dna: betelgeuse_balances::<Instance1>::{Module, Call, Storage, Config<T>, Event<T>},
		TransactionPayment: pallet_transaction_payment::{Module, Storage},

		// Consensus support.
		Authorship: pallet_authorship::{Module, Call, Storage, Inherent},
		Staking: betelgeuse_staking::{Module, Call, Storage, Config<T>, Event<T>, ValidateUnsigned},
		...
		Democracy: betelgeuse_democracy::{Module, Call, Storage, Config, Event<T>},
		...
		ElectionsPhragmen: betelgeuse_elections_phragmen::{Module, Call, Storage, Config<T>, Event<T>},
		TechnicalMembership: pallet_membership::<Instance0>::{Module, Call, Storage, Config<T>, Event<T>},
		Treasury: betelgeuse_treasury::{Module, Call, Storage, Event<T>},

		// Claims. Usable initially.
		Claims: betelgeuse_claims::{Module, Call, Storage, Config, Event<T>, ValidateUnsigned},

		// Vesting. Usable initially, but removed once all vesting is finished.
		Vesting: betelgeuse_vesting::{Module, Call, Storage, Event<T>, Config<T>},
		...
		CrabIssuing: betelgeuse_crab_issuing::{Module, Call, Storage, Config, Event<T>},
		CrabBacking: betelgeuse_crab_backing::{Module, Storage, Config<T>},

		EthereumRelayerGame: betelgeuse_relayer_game::<Instance0>::{Module, Storage},
		EthereumRelay: betelgeuse_ethereum_relay::{Module, Call, Storage, Config<T>, Event<T>},
		EthereumBacking: betelgeuse_ethereum_backing::{Module, Call, Storage, Config<T>, Event<T>},

		TronBacking: betelgeuse_tron_backing::{Module, Storage, Config<T>},

		HeaderMMR: betelgeuse_header_mmr::{Module, Call, Storage},

		EVM: betelgeuse_evm::{Module, Config, Call, Storage, Event<T>},
		Ethereum: dvm_ethereum::{Module, Call, Storage, Event, Config, ValidateUnsigned},

		EthereumRelayAuthorities: betelgeuse_relay_authorities::<Instance0>::{Module, Call, Storage, Config<T>, Event<T>},
	}
);
	...
	Call: From<LocalCall>,
{
	fn create_transaction<C: frame_system::offchain::AppCrypto<Self::Public, Self::Signature>>(
		...
		let extra: SignedExtra = (
			...
			betelgeuse_ethereum_relay::CheckEthereumRelayHeaderParcel::<Runtime>::new(),
		);
...
impl_runtime_apis! {
	...
	impl betelgeuse_balances_rpc_runtime_api::BalancesApi<Block, AccountId, Balance> for Runtime {
		fn usable_balance(instance: u8, account: AccountId) -> BalancesRuntimeDispatchInfo<Balance> {
			match instance {
				0 => Etp3::usable_balance_rpc(account),
	...
	impl betelgeuse_header_mmr_rpc_runtime_api::HeaderMMRApi<Block, Hash> for Runtime {
		fn gen_proof(
	...
	impl betelgeuse_staking_rpc_runtime_api::StakingApi<Block, AccountId, Power> for Runtime {
	...
	impl dvm_rpc_runtime_api::EthereumRuntimeRPCApi<Block> for Runtime {
		fn chain_id() -> u64 {
			<Runtime as betelgeuse_evm::Trait>::ChainId::get()
		}

		fn gas_price() -> U256 {
			<Runtime as betelgeuse_evm::Trait>::FeeCalculator::min_gas_price()
		}

		fn account_basic(address: H160) -> EVMAccount {
			<Runtime as betelgeuse_evm::Trait>::AccountBasicMapping::account_basic(&address)
		}

		fn account_code_at(address: H160) -> Vec<u8> {
			betelgeuse_evm::Module::<Runtime>::account_codes(address)
		}
		...
		fn storage_at(address: H160, index: U256) -> H256 {
			...
			betelgeuse_evm::Module::<Runtime>::account_storages(address, H256::from_slice(&tmp[..]))
		}
		...
		fn call(
			...
			nonce: Option<U256>,
			estimate: bool,
		) -> Result<betelgeuse_evm::CallInfo, sp_runtime::DispatchError> {
			let config = if estimate {
				let mut config = <Runtime as betelgeuse_evm::Trait>::config().clone();
			...
			<Runtime as betelgeuse_evm::Trait>::Runner::call(
				from,
				to,
				data,
				value,
				...
				gas_price,
				nonce,
				config.as_ref().unwrap_or(<Runtime as betelgeuse_evm::Trait>::config()),
		...
		fn create(
			...
			nonce: Option<U256>,
			estimate: bool,
		) -> Result<betelgeuse_evm::CreateInfo, sp_runtime::DispatchError> {
			let config = if estimate {
				let mut config = <Runtime as betelgeuse_evm::Trait>::config().clone();
				config.estimate = true;
				Some(config)
			...
			<Runtime as betelgeuse_evm::Trait>::Runner::create(
				from,
				data,
				value,
				gas_limit.low_u32(),
